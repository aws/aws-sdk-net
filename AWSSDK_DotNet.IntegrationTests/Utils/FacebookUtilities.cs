...
namespace AWSSDK_DotNet.IntegrationTests.Utils
{
    public static class FacebookUtilities
    {
        ...
        public static FacebookCreateUserResponse CreateFacebookUser(string facebookAppId, string facebookAppSecret)
        {
            ...
            using (var createUserResponse = createUserRequest.GetResponse())
            {
        ...
        private static string GetFacebookAccessToken(string facebookAppId, string facebookAppSecret)
        {
            ...
            using (var accessTokenResponse = accessTokenRequest.GetResponse())
            {
                using (var streamReader = new StreamReader(accessTokenResponse.GetResponseStream(), Encoding.GetEncoding(1252)))
                {
