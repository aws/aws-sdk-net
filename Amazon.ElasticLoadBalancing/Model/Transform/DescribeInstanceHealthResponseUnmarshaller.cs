...
namespace Amazon.ElasticLoadBalancing.Model.Transform
{
    ...
    /// </summary>
    internal class DescribeInstanceHealthResponseUnmarshaller : IResponseUnmarshaller<DescribeInstanceHealthResponse, UnmarshallerContext> {

        public DescribeInstanceHealthResponse Unmarshall(UnmarshallerContext context) 
        {
            DescribeInstanceHealthResponse response = new DescribeInstanceHealthResponse();

            while (context.Read())
            {
                if (context.IsStartElement)
                {
                    if(context.TestExpression("DescribeInstanceHealthResult", 2))
                    {
                        response.DescribeInstanceHealthResult = DescribeInstanceHealthResultUnmarshaller.GetInstance().Unmarshall(context);
                        continue;
                    }
        ...
        private static DescribeInstanceHealthResponseUnmarshaller instance;

        public static DescribeInstanceHealthResponseUnmarshaller GetInstance()
        {
            if (instance == null) 
            {
               instance = new DescribeInstanceHealthResponseUnmarshaller();
            }
            return instance;
        }
