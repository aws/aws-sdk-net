...
namespace Amazon.ElasticLoadBalancing.Model.Internal.MarshallTransformations
{
    ...
    /// </summary>
    internal class CreateAppCookieStickinessPolicyResponseUnmarshaller : IResponseUnmarshaller<CreateAppCookieStickinessPolicyResponse, UnmarshallerContext> {

        public CreateAppCookieStickinessPolicyResponse Unmarshall(UnmarshallerContext context) 
        {
            CreateAppCookieStickinessPolicyResponse response = new CreateAppCookieStickinessPolicyResponse();

            while (context.Read())
            {
                if (context.IsStartElement)
                {
                    if(context.TestExpression("CreateAppCookieStickinessPolicyResult", 2))
                    {
                        response.CreateAppCookieStickinessPolicyResult = CreateAppCookieStickinessPolicyResultUnmarshaller.GetInstance().Unmarshall(context);
                        continue;
                    }
        ...
        private static CreateAppCookieStickinessPolicyResponseUnmarshaller instance;

        public static CreateAppCookieStickinessPolicyResponseUnmarshaller GetInstance()
        {
            if (instance == null) 
            {
               instance = new CreateAppCookieStickinessPolicyResponseUnmarshaller();
            }
            return instance;
        }
