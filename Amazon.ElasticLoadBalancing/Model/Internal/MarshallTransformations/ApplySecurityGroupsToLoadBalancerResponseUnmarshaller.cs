...
namespace Amazon.ElasticLoadBalancing.Model.Internal.MarshallTransformations
{
    ...
    /// </summary>
    internal class ApplySecurityGroupsToLoadBalancerResponseUnmarshaller : IResponseUnmarshaller<ApplySecurityGroupsToLoadBalancerResponse, UnmarshallerContext> {

        public ApplySecurityGroupsToLoadBalancerResponse Unmarshall(UnmarshallerContext context) 
        {
            ApplySecurityGroupsToLoadBalancerResponse response = new ApplySecurityGroupsToLoadBalancerResponse();

            while (context.Read())
            {
                if (context.IsStartElement)
                {
                    if(context.TestExpression("ApplySecurityGroupsToLoadBalancerResult", 2))
                    {
                        response.ApplySecurityGroupsToLoadBalancerResult = ApplySecurityGroupsToLoadBalancerResultUnmarshaller.GetInstance().Unmarshall(context);
                        continue;
                    }
        ...
        private static ApplySecurityGroupsToLoadBalancerResponseUnmarshaller instance;

        public static ApplySecurityGroupsToLoadBalancerResponseUnmarshaller GetInstance()
        {
            if (instance == null) 
            {
               instance = new ApplySecurityGroupsToLoadBalancerResponseUnmarshaller();
            }
            return instance;
        }
