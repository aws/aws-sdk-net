...
namespace Amazon.Extensions.NETCore.Setup
{
    ...
    internal class ClientFactory
    {
        ...
        internal object CreateServiceClient(IServiceProvider provider)
        {
            ...
            // Run any create event handlers
            var handles = provider.GetService<IAmazonServiceClientCreateEventHandlers>();
            if (handles != null)
            {
                foreach (var handle in handles.Handlers)
                {
                    logger?.LogInformation($"Executing Create Service Client Event Handle {handle.Name} with {client.GetType().FullName}");
                    client = handle.Process(client);
                }
            }
