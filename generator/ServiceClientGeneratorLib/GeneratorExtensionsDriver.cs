...
namespace ServiceClientGenerator
{
    public class GeneratorExtensionsDriver
    {
        ...
        public GeneratorExtensionsDriver(ExtensionConfiguration config, GenerationManifest generationManifest, GeneratorOptions options)
        {
        ...
        /// <summary>
        /// Validates bidirectional consistency between AWSSDK dependencies in nuspec and ServiceDependencies configuration.
        /// </summary>
        public void Validate()
        {
            if (!File.Exists(_nuspecPath) || Configuration.ServiceDependencies == null)
                return;
            ...
            var configServices = new HashSet<string>(Configuration.ServiceDependencies.Keys);

            foreach (var service in nuspecServices)
            {
                if (!configServices.Contains(service))
                    throw new Exception($"AWSSDK.{service} dependency found in nuspec but missing from ServiceDependencies");
            }

            foreach (var service in configServices)
            {
                if (!nuspecServices.Contains(service))
                    throw new Exception($"{service} found in ServiceDependencies but AWSSDK.{service} dependency missing from nuspec");
            }
        }
        ...
        public void ProcessNuspec()
        {
            ...
            foreach (XmlNode node in dependencyNodes)
            {
                ...
                if (id.StartsWith("AWSSDK."))
                {
                    ...
                    if (Configuration.ServiceDependencies.ContainsKey(serviceName))
                    {
                        if(node.Attributes["version"].Value != Configuration.ServiceDependencies[serviceName])
                        {
                            node.Attributes["version"].Value = Configuration.ServiceDependencies[serviceName];
