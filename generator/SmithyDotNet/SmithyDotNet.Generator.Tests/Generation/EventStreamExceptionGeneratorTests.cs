...
public sealed class EventStreamExceptionGeneratorTests : IDisposable
{
    ...
    {
        Assert.Contains(ExceptionPath, Generate("Codegen/event-stream-model.json"));
    }

    [Fact]
    public void InputOnlyEventStream_EmitsNoException()
    {
        Assert.DoesNotContain(ExceptionPath, Generate("Codegen/event-stream-input-only-model.json"));
    }

    [Fact]
    public void EventStreamExceptionMembers_EmitOneExceptionAndUnmarshallerEach()
    {
        var written = Generate("Codegen/event-stream-model.json");
