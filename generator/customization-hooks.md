...
### Type: string[], bool
- Description: These two customization hooks are often used together to skip the unmarshalling logic within an xml `testExpression` block and to inject custom code instead. Set `skipContextTestExpressionUnmarshallingLogic` to `true` and speciy what you want to inject via `injectXmlUnmarshallResultCode`. Though an array of code can be injected, it is best practice to inject a method and in a custom partial class, define that method. 
- Example Usage
```json
"shapeModifiers":{
...
### Type: string[]
- Description: This customization hook is used for operations where code needs to injected in the unmarshallResult method and isn't tied to a specific member. This customization hook will inject code right before the `context.Read()` call. 
- Example Usage
```json
"shapeModifiers":{
...
### Type: string[]
- Description: Though the name suggests that this can only be used for XML based services, this isn't true. Thsi can be used for any protocol. This customization hook is used to inject code into the getter of a property. This will completely replace the getter with the code specified in the string array.
- Example Usage
```json
"shapeModifiers":{
    "S3Grantee":{
...
### Type: string[]
- Description: Though the name suggests that this can only be used for XML based services, this isn't true. Thsi can be used for any protocol. This customization hook is used to inject code into the setter of a property. This will completely replace the setter with the code specified in the string array
- Example Usage
```json
"shapeModifiers":{
    "CopyObjectRequest":{
...
### Name: mergedEnums
### Type: object
- Description: Use this customization hook to speicy that an enum should be merged into existing enums instead of being generated separately.
- Example Usage
```json
    "mergedEnums": {
        "MFADeleteStatus": {
...
### Type: bool
- Description: Use this customization hook to skip setting the checksum during the marshalling. If an operation has custom logic for setting checksums this customization hook can be used. It can also be used to completely skip setting the checksum, although that should rarely be the case.
- Example usage:
```json
"operationModifiers":{
    "UploadPart": {
...
### Type: string[]
- Description: Use this customization hook to exclude the result keys specific in the result_keys in the paginator model
- Example usage:
```json
"operationModifiers":{
    "ListVersions":{
