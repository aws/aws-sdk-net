        ...
        static int Main(string[] args)
        {
            ...
            try
            {
                ...
                if (string.IsNullOrEmpty(options.SelfServiceModel))
                {
                    ...
                    // Process extensions
                    Parallel.ForEach(generationManifest.ExtensionConfigurations, new ParallelOptions { MaxDegreeOfParallelism = Environment.ProcessorCount * 2 }, (extensionConfig, state) =>
                    {
                        Console.WriteLine("Processing extension: {0} ({1})", extensionConfig.Id,
                            extensionConfig.Path);

                        var driver = new GeneratorExtensionsDriver(extensionConfig, generationManifest, options);
                        driver.Execute();
                    });
