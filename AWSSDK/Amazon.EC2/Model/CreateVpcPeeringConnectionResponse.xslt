...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:ec2="default-ec2-namespace" exclude-result-prefixes="ec2">
  ...
  <xsl:template match="ec2:CreateVpcPeeringConnectionResponse">
    <xsl:element name="CreateVpcPeeringConnectionResponse">
      <xsl:element name="ResponseMetadata">
        <xsl:element name="RequestId">
          <xsl:value-of select="ec2:requestId"/>
        </xsl:element>
      </xsl:element>
      <xsl:element name="CreateVpcPeeringConnectionResult">
        <xsl:element name="VpcPeeringConnection">
          <xsl:apply-templates select="ec2:vpcPeeringConnection"/>
        </xsl:element>
      </xsl:element>
    </xsl:element>
  </xsl:template>
  <xsl:template match="ec2:vpcPeeringConnection">
