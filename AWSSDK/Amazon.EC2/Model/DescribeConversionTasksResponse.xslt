...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:ec2="http://ec2.amazonaws.com/doc/2012-06-15/" exclude-result-prefixes="ec2">
  ...
  <xsl:template match="ec2:DescribeConversionTasksResponse">
    <xsl:element name="DescribeConversionTasksResponse" namespace="{$ns}">
      <xsl:element name="ResponseMetadata" namespace="{$ns}">
        <xsl:element name="RequestId" namespace="{$ns}">
          <xsl:value-of select="ec2:requestId"/>
        </xsl:element>
      </xsl:element>
      <xsl:element name="DescribeConversionTasksResult" namespace="{$ns}">

        <xsl:apply-templates select="ec2:conversionTasks" />
  ...
  <xsl:template match="ec2:conversionTasks">
    <xsl:for-each select="ec2:item">

      <!-- ConversionTaskType -->
      <xsl:element name="ConversionTasks" namespace="{$ns}">
