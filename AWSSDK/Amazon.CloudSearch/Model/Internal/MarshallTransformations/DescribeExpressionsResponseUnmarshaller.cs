...
{
    /// <summary>
    ///    Response Unmarshaller for DescribeExpressions operation
    /// </summary>
    internal class DescribeExpressionsResponseUnmarshaller : XmlResponseUnmarshaller
    {

        public override AmazonWebServiceResponse Unmarshall(XmlUnmarshallerContext context) 
        {
            DescribeExpressionsResponse response = new DescribeExpressionsResponse();
            
            while (context.Read())
            {
                if (context.IsStartElement)                
                {
                    if(context.TestExpression("DescribeExpressionsResult", 2))
                    {
                        response.DescribeExpressionsResult = DescribeExpressionsResultUnmarshaller.GetInstance().Unmarshall(context);
                        continue;
                    }
        ...
        private static DescribeExpressionsResponseUnmarshaller instance;

        public static DescribeExpressionsResponseUnmarshaller GetInstance()
        {
            if (instance == null) 
            {
               instance = new DescribeExpressionsResponseUnmarshaller();
            }
            return instance;
        }
