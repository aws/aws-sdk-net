...
{
    /// <summary>
    ///    Response Unmarshaller for UpdateStopwordOptions operation
    /// </summary>
    internal class UpdateStopwordOptionsResponseUnmarshaller : XmlResponseUnmarshaller
    {

        public override AmazonWebServiceResponse Unmarshall(XmlUnmarshallerContext context) 
        {
            UpdateStopwordOptionsResponse response = new UpdateStopwordOptionsResponse();
            
            while (context.Read())
            {
                if (context.IsStartElement)                
                {
                    if(context.TestExpression("UpdateStopwordOptionsResult", 2))
                    {
                        response.UpdateStopwordOptionsResult = UpdateStopwordOptionsResultUnmarshaller.GetInstance().Unmarshall(context);
                        continue;
                    }
        ...
        private static UpdateStopwordOptionsResponseUnmarshaller instance;
