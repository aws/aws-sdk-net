    ...
    internal class MultiBatchGet
    {
        ...
        private Dictionary<string, RequestSet> ConvertBatches()
        {
            ...
            foreach (var batch in Batches)
            {
                ...
                {
                    var keys = new RequestSet(batch.Keys, batch);
        ...
        private class RequestSet : QuickList<Key>
        {
            public DocumentBatchGet Batch { get; private set; }

            public RequestSet(DocumentBatchGet batch)
                : this(new Key[0], batch)
            { }

            public RequestSet(IEnumerable<Key> items, DocumentBatchGet batch)
                : base(items)
            {
                Batch = batch;
            }
