...
namespace Amazon.IdentityManagement.Model.Internal.MarshallTransformations
{
    ...
    /// </summary>
    internal class CreateInstanceProfileResponseUnmarshaller : XmlResponseUnmarshaller
    {

        public override AmazonWebServiceResponse Unmarshall(XmlUnmarshallerContext context) 
        {
            CreateInstanceProfileResponse response = new CreateInstanceProfileResponse();
            
            while (context.Read())
            {
                if (context.IsStartElement)                
                {
                    if(context.TestExpression("CreateInstanceProfileResult", 2))
                    {
                        response.CreateInstanceProfileResult = CreateInstanceProfileResultUnmarshaller.GetInstance().Unmarshall(context);
                        continue;
                    }
        ...
        private static CreateInstanceProfileResponseUnmarshaller instance;

        public static CreateInstanceProfileResponseUnmarshaller GetInstance()
        {
            if (instance == null) 
            {
               instance = new CreateInstanceProfileResponseUnmarshaller();
            }
            return instance;
        }
