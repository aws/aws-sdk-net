...
namespace Amazon.Glacier.Model
{
    ...
    /// a            job to retrieve either an archive or a vault inventory (a list of archives in
    /// a vault). 
    /// 
    ///         
    /// <para>
    ...
    /// </li>        </ol>        
    /// <para>
    /// The retrieval request is executed asynchronously. When you initiate a retrieval job,            Amazon
    /// Glacier creates a job and returns a job ID in the response. When Amazon Glacier completes            the
    /// job, you can get the job output (archive or inventory data). For information about            getting
    /// job output, see <a>GetJobOutput</a> operation. 
    /// </para>
    ///         
    /// <para>
    ...
    /// Service (Amazon SNS) topic to which Amazon Glacier can                        post a notification after
    /// the job is completed. You can specify an SNS topic                        per job request. The notification
    /// is sent only after Amazon Glacier completes the                        job. In addition to specifying
    /// an SNS topic per job request, you can                        configure vault notifications for a vault
    /// so that job notifications are                        always sent. For more information, see <a>SetVaultNotifications</a>.
    /// </para>
    ///             </li>            <li>
    /// <para>
    /// <b>Get job details</b> You can make a <a>DescribeJob</a> request                        to obtain job
    /// status information while a job is in progress. However, it is                        more efficient
    ...
    /// and also            specify an SNS topic in your initiate job request, Amazon Glacier sends
    /// both            notifications.  For more information, see <a>SetVaultNotifications</a>.
    /// </para>
    ///         
    /// <para>
    ...
    /// inventory for                the vault. The inventory data you get might be up to a day or two
    /// days old. Also,                the initiate inventory job might take some time to complete before
    /// you can download                the vault inventory. So you do not want to retrieve a vault inventory
    /// for each vault                operation. However, in some scenarios, you might find the vault
    /// inventory useful.                For example, when you upload an archive, you can provide an archive
    /// description but                not an archive name. Amazon Glacier provides you a unique archive
    /// ID, an opaque                string of characters. So, you might maintain your own database that
    /// maps archive                names to their corresponding Amazon Glacier assigned archive IDs.
    /// You might find the                vault inventory useful in the event you need to reconcile information
    ...
    /// and               <code>EndDate</code> by specifying values for these parameters in
    /// the                <b>InitiateJob</b> request.                Archives created on
    /// or after the <code>StartDate</code> and before the <code>EndDate</code> will be returned.
    /// If               you only provide the <code>StartDate</code> without the <code>EndDate</code>,
    /// you will retrieve the inventory for               all archives created on or after
    /// the <code>StartDate</code>. If you only provide the <code>EndDate</code>         
    ///      without the <code>StartDate</code>, you will get back the inventory for all archives
    /// created               before the <code>EndDate</code>.
    /// </para>
    ///                     
    /// <para>
    ...
    ///         parameter in the <b>InitiateJob</b> request. The         inventory job output
    /// will contain inventory items up to the specified         <code>Limit</code>. If there
    /// are more inventory items available, the result         is paginated. After a job is
    /// complete you can use the <a>DescribeJob</a>          operation to get a marker that
    /// you use in a subsequent <b>InitiateJob</b> request.          The marker will indicate
    /// the starting point to retrieve the next set of inventory items. You can page through
    /// your         entire inventory by repeatedly making <b>InitiateJob</b> requests with
    /// the marker from the previous          <b>DescribeJob</b> output, until you get a marker
    /// from <b>DescribeJob</b> that returns null, indicating         that there are no more
    ...
    /// archive. If the ranged archive retrieval is not megabyte aligned, this operation returns
    /// a          400 response. Furthermore, to ensure you get checksum values for data you
    /// download using          Get Job Output API, the range must be tree hash aligned. 
    ///               
    /// </para>
    ///         
    /// <para>
    /// An AWS account has full permission to perform all operations (actions). However, AWS
    /// Identity and Access Management (IAM) users don't have any permissions by default.
    ///            You must grant them explicit permission to perform specific actions. For more
    /// information,            see <a href="http://docs.aws.amazon.com/amazonglacier/latest/dev/using-iam-with-amazon-glacier.html">Access
    /// Control Using AWS Identity and Access Management (IAM)</a>.
    /// </para>
    ///      
    /// <para>
