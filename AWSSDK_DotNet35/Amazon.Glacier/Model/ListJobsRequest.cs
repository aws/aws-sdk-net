...
namespace Amazon.Glacier.Model
{
    ...
    /// be                    retrieved. Retaining completed jobs for a period of time after they have                    completed
    /// enables you to get a job output in the event you miss the job                    completion notification
    /// or your first attempt to download it fails. For example,                    suppose you start an
    /// archive retrieval job to download an archive. After the job                    completes, you start
    /// to download the archive but encounter a network error. In                    this scenario, you can
    /// retry and download the archive while the job exists. 
    /// </para>
    /// </note>                
    /// <para>
    ...
    /// retrieval, the            output is the archive data, and for an inventory retrieval, it is
    /// the inventory list.            The List Job operation returns a list of these jobs sorted by
    /// job initiation            time.
    /// </para>
    ///         
    /// <para>
    /// This List Jobs operation supports pagination.            By default, this operation returns
    /// up to 1,000 jobs in the response.            You should always check the response for a
    ...
    /// begins            at a specific job, set the <code>marker</code> request parameter to the value
    /// you            obtained from a previous List Jobs request. You can also limit the number of
    /// jobs            returned in the response by specifying the <code>limit</code> parameter in
    /// the            request.
    /// </para>
    ///         
    /// <para>
    ...
    /// status are returned. The                <code>completed</code> parameter allows you to specify
    ...
    public partial class ListJobsRequest : AmazonGlacierRequest
    {
        ...
        /// of jobs should begin.          Get the marker value from a previous List Jobs response.
        ///          You need only include the marker if you are continuing the pagination of
