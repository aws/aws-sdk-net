    ...
    public class CopyPartRequestMarshaller : IMarshaller<IRequest, CopyPartRequest>
    {
        ...
        public IRequest Marshall(CopyPartRequest copyPartRequest)
        {
            ...
            if (copyPartRequest.IsSetModifiedSinceDate())
                request.Headers.Add("x-amz-copy-source-if-modified-since", copyPartRequest.ModifiedSinceDate.ToUniversalTime().ToString(AWSSDKUtils.GMTDateFormat));

            if (copyPartRequest.IsSetUnmodifiedSinceDate())
                request.Headers.Add("x-amz-copy-source-if-unmodified-since", copyPartRequest.UnmodifiedSinceDate.ToUniversalTime().ToString(AWSSDKUtils.GMTDateFormat));
        ...
        string ConstructCopySourceRangeHeader(long firstByte, long lastByte)
        {
            return string.Format("bytes={0}-{1}", firstByte, lastByte);
        }
    }
}
