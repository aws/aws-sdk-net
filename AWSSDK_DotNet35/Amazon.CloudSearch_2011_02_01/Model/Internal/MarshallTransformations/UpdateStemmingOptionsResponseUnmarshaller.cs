...
{
    /// <summary>
    ///    Response Unmarshaller for UpdateStemmingOptions operation
    /// </summary>
    internal class UpdateStemmingOptionsResponseUnmarshaller : XmlResponseUnmarshaller
    {
        public override AmazonWebServiceResponse Unmarshall(XmlUnmarshallerContext context) 
        {   
            UpdateStemmingOptionsResponse response = new UpdateStemmingOptionsResponse();
            
            while (context.Read())
            {
                if (context.IsStartElement)
                {                    
                    if(context.TestExpression("UpdateStemmingOptionsResult", 2))
                    {
        ...
        private static void UnmarshallResult(XmlUnmarshallerContext context,UpdateStemmingOptionsResponse response)
        {
            ...
            while (context.Read())
            {
                if (context.IsStartElement || context.IsAttribute)
                {
                    ...
                    {
                        response.Stems = StemmingOptionsStatusUnmarshaller.GetInstance().Unmarshall(context);
        ...
        private static UpdateStemmingOptionsResponseUnmarshaller instance;
