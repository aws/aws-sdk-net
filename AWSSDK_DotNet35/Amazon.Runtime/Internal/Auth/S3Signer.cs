...
namespace Amazon.Runtime.Internal.Auth
{
    internal class S3Signer : AbstractAWSSigner
    {
        ...
        string buildSigningString(string verb, string canonicalizedResource, IDictionary<string, string> parameters, IDictionary<string,string> webHeaders)
        {
            ...
            {
                int quotePos = canonicalizedResource.IndexOf("?");
                if (quotePos != -1)
                {
        ...
        StringBuilder buildCanonicalizedHeaders(IDictionary<string, string> headers)
        {
            // Create the canonicalized header string to return.
            StringBuilder sb = new StringBuilder(256);
            foreach (string key in headers.Keys.OrderBy(x => x.ToLower()))
            {
