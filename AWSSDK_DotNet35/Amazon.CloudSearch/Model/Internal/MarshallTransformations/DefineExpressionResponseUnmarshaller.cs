...
{
    /// <summary>
    ///    Response Unmarshaller for DefineExpression operation
    /// </summary>
    internal class DefineExpressionResponseUnmarshaller : XmlResponseUnmarshaller
    {
        public override AmazonWebServiceResponse Unmarshall(XmlUnmarshallerContext context) 
        {   
            DefineExpressionResponse response = new DefineExpressionResponse();
            
            while (context.Read())
            {
                if (context.IsStartElement)
                {                    
                    if(context.TestExpression("DefineExpressionResult", 2))
                    {
        ...
        private static void UnmarshallResult(XmlUnmarshallerContext context,DefineExpressionResponse response)
        {
        ...
        private static DefineExpressionResponseUnmarshaller instance;

        public static DefineExpressionResponseUnmarshaller GetInstance()
        {
            if (instance == null) 
            {
               instance = new DefineExpressionResponseUnmarshaller();
            }
            return instance;
        }
