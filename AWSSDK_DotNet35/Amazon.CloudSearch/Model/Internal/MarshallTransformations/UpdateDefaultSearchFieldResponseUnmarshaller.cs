...
{
    /// <summary>
    ///    Response Unmarshaller for UpdateDefaultSearchField operation
    /// </summary>
    internal class UpdateDefaultSearchFieldResponseUnmarshaller : XmlResponseUnmarshaller
    {
        public override AmazonWebServiceResponse Unmarshall(XmlUnmarshallerContext context) 
        {   
            UpdateDefaultSearchFieldResponse response = new UpdateDefaultSearchFieldResponse();
            
            while (context.Read())
            {
                if (context.IsStartElement)
                {                    
                    if(context.TestExpression("UpdateDefaultSearchFieldResult", 2))
                    {
        ...
        private static void UnmarshallResult(XmlUnmarshallerContext context,UpdateDefaultSearchFieldResponse response)
        {
            ...
            while (context.Read())
            {
                if (context.IsStartElement || context.IsAttribute)
                {
                    ...
                    {
                        response.DefaultSearchField = DefaultSearchFieldStatusUnmarshaller.GetInstance().Unmarshall(context);
        ...
        private static UpdateDefaultSearchFieldResponseUnmarshaller instance;

        public static UpdateDefaultSearchFieldResponseUnmarshaller GetInstance()
        {
            if (instance == null) 
            {
               instance = new UpdateDefaultSearchFieldResponseUnmarshaller();
            }
            return instance;
        }
