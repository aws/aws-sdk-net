...
{
    /// <summary>
    ///    Response Unmarshaller for DeleteIndexField operation
    /// </summary>
    internal class DeleteIndexFieldResponseUnmarshaller : XmlResponseUnmarshaller
    {
        public override AmazonWebServiceResponse Unmarshall(XmlUnmarshallerContext context) 
        {   
            DeleteIndexFieldResponse response = new DeleteIndexFieldResponse();
            
            while (context.Read())
            {
                if (context.IsStartElement)
                {                    
                    if(context.TestExpression("DeleteIndexFieldResult", 2))
                    {
        ...
        private static void UnmarshallResult(XmlUnmarshallerContext context,DeleteIndexFieldResponse response)
        {
            ...
            while (context.Read())
            {
                if (context.IsStartElement || context.IsAttribute)
                {
                    ...
                    {
                        response.IndexField = IndexFieldStatusUnmarshaller.GetInstance().Unmarshall(context);
        ...
        private static DeleteIndexFieldResponseUnmarshaller instance;

        public static DeleteIndexFieldResponseUnmarshaller GetInstance()
        {
            if (instance == null) 
            {
               instance = new DeleteIndexFieldResponseUnmarshaller();
            }
            return instance;
        }
