...
{
    /// <summary>
    ///    Response Unmarshaller for DeleteExpression operation
    /// </summary>
    internal class DeleteExpressionResponseUnmarshaller : XmlResponseUnmarshaller
    {
        public override AmazonWebServiceResponse Unmarshall(XmlUnmarshallerContext context) 
        {   
            DeleteExpressionResponse response = new DeleteExpressionResponse();
            
            while (context.Read())
            {
                if (context.IsStartElement)
                {                    
                    if(context.TestExpression("DeleteExpressionResult", 2))
                    {
        ...
        private static void UnmarshallResult(XmlUnmarshallerContext context,DeleteExpressionResponse response)
        {
            ...
            while (context.Read())
            {
                if (context.IsStartElement || context.IsAttribute)
                {
                    ...
                    {
                        response.Expression = ExpressionStatusUnmarshaller.GetInstance().Unmarshall(context);
        ...
        private static DeleteExpressionResponseUnmarshaller instance;

        public static DeleteExpressionResponseUnmarshaller GetInstance()
        {
            if (instance == null) 
            {
               instance = new DeleteExpressionResponseUnmarshaller();
            }
            return instance;
        }
