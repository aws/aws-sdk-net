...
namespace Amazon.IdentityManagement.Model.Internal.MarshallTransformations
{
    ...
    /// </summary>
    internal class GetSAMLProviderResponseUnmarshaller : XmlResponseUnmarshaller
    {
        ...
        private static void UnmarshallResult(XmlUnmarshallerContext context,GetSAMLProviderResponse response)
        {
            ...
            while (context.Read())
            {
                if (context.IsStartElement || context.IsAttribute)
                {
                    ...
                    {
                        response.SAMLMetadataDocument = StringUnmarshaller.GetInstance().Unmarshall(context);
                            
                        continue;
                    }
                    if (context.TestExpression("CreateDate", targetDepth))
                    {
                        response.CreateDate = DateTimeUnmarshaller.GetInstance().Unmarshall(context);
                            
                        continue;
                    }
                    if (context.TestExpression("ValidUntil", targetDepth))
                    {
                        response.ValidUntil = DateTimeUnmarshaller.GetInstance().Unmarshall(context);
        ...
        private static GetSAMLProviderResponseUnmarshaller instance;

        public static GetSAMLProviderResponseUnmarshaller GetInstance()
        {
            if (instance == null) 
            {
               instance = new GetSAMLProviderResponseUnmarshaller();
            }
            return instance;
        }
