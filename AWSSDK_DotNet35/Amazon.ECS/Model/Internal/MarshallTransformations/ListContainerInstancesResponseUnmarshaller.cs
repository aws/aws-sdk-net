...
namespace Amazon.ECS.Model.Internal.MarshallTransformations
{
    ...
    /// </summary>  
    public class ListContainerInstancesResponseUnmarshaller : XmlResponseUnmarshaller
    {
        public override AmazonWebServiceResponse Unmarshall(XmlUnmarshallerContext context)
        {
            ...
            while (context.ReadAtDepth(targetDepth))
            {
                ...
                {                    
                    if(context.TestExpression("ListContainerInstancesResult", 2))
                    {
        ...
        private static void UnmarshallResult(XmlUnmarshallerContext context, ListContainerInstancesResponse response)
        {
            ...
            while (context.ReadAtDepth(originalDepth))
            {
                if (context.IsStartElement || context.IsAttribute)
                {
                    ...
                    if (context.TestExpression("containerInstanceArns/member", targetDepth))
                    {
                        var unmarshaller = StringUnmarshaller.Instance;
                        var item = unmarshaller.Unmarshall(context);
                        response.ContainerInstanceArns.Add(item);
                        continue;
                    }
