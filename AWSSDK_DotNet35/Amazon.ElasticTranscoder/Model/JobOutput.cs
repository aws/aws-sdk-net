...
namespace Amazon.ElasticTranscoder.Model
{
    ...
    /// for a job, the                <code>Output</code> object lists information about the
    /// first output. This duplicates            the information that is listed for the first
    /// output in the <code>Outputs</code>            object.
    /// </summary>
    public partial class JobOutput
    {
        ...
        /// only),               <code>CEA-708 (EIA-708</code>, first non-empty channel only),
        ...
        ///                <code>ts</code> (MPEG-TS), <code>SegmentDuration</code> is the maximum
        /// duration of            each .ts file in seconds. The range of valid values is 1 to
        /// 60 seconds. If the duration            of the video is not evenly divisible by <code>SegmentDuration</code>,
        /// the duration of            the last segment is the remainder of total length/SegmentDuration.
        /// Elastic Transcoder            creates an output-specific playlist for each output
        /// that you specify in OutputKeys. To            add an output to the master playlist
        /// for this job, include it in            <code>OutputKeys</code>.
        /// </para>
        /// </summary>
        ...
        ///                <code>Outputs:Status</code> is always the same as <code>Job:Status</code>.
        /// If you            specified more than one output: <ul>                <li><code>Job:Status</code>
        /// and <code>Outputs:Status</code> for all of the outputs                    is Submitted
        /// until Elastic Transcoder starts to process the first output.</li>                <li>When
        /// Elastic Transcoder starts to process the first output,                        <code>Outputs:Status</code>
        /// for that output and <code>Job:Status</code> both                    change to Progressing.
        /// For each output, the value of <code>Outputs:Status</code>                    remains
        /// Submitted until Elastic Transcoder starts to process the output.</li>            
        ///    <li>Job:Status remains Progressing until all of the outputs reach a terminal status,
        ///                    either Complete or Error.</li>                <li>When all of the
        /// outputs reach a terminal status, <code>Job:Status</code> changes                 
        ///   to Complete only if <code>Outputs:Status</code> for all of the outputs is      
        ///                  <code>Complete</code>. If <code>Outputs:Status</code> for one or
        /// more                    outputs is <code>Error</code>, the terminal status for <code>Job:Status</code>
        ///                    is also <code>Error</code>.</li>            </ul> The value of
        /// <code>Status</code> is one of the following: <code>Submitted</code>,             
        ...
        /// Settings for each            watermark must be defined in the preset that you specify
        /// in <code>Preset</code> for the            current output.
        /// </para>
        ///         
        /// <para>
        /// Watermarks are added to the output video in the sequence in which you list them in
        /// the            job output&#x2014;the first watermark in the list is added to the output
        /// video first, the            second watermark in the list is added next, and so on.
        /// As a result, if the settings in a            preset cause Elastic Transcoder to place
        /// all watermarks in the same location, the second watermark            that you add
        /// will cover the first one, the third one will cover the second, and the           
