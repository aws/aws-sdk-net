...
namespace AWSSDK_DotNet35.UnitTests.PaginatorTests
{
    ...
    public class IoTWirelessPaginatorTests
    {
        ...
        [TestCategory("UnitTest")]
        [TestCategory("IoTWireless")]
        public void ListQueuedMessagesTest_TwoPages()
        {
            var request = InstantiateClassGenerator.Execute<ListQueuedMessagesRequest>();

            var firstResponse = InstantiateClassGenerator.Execute<ListQueuedMessagesResponse>();
            var secondResponse = InstantiateClassGenerator.Execute<ListQueuedMessagesResponse>();
            secondResponse.NextToken = null;

            _mockClient.SetupSequence(x => x.ListQueuedMessages(request)).Returns(firstResponse).Returns(secondResponse);
            var paginator = _mockClient.Object.Paginators.ListQueuedMessages(request);
        ...
        [TestCategory("IoTWireless")]
        [ExpectedException(typeof(System.InvalidOperationException), "Paginator has already been consumed and cannot be reused. Please create a new instance.")]
        public void ListQueuedMessagesTest__OnlyUsedOnce()
        {
            var request = InstantiateClassGenerator.Execute<ListQueuedMessagesRequest>();

            var response = InstantiateClassGenerator.Execute<ListQueuedMessagesResponse>();
            response.NextToken = null;

            _mockClient.Setup(x => x.ListQueuedMessages(request)).Returns(response);
            var paginator = _mockClient.Object.Paginators.ListQueuedMessages(request);
