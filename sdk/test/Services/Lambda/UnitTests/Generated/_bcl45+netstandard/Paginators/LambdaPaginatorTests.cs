...
namespace AWSSDK_DotNet35.UnitTests.PaginatorTests
{
    ...
    public class LambdaPaginatorTests
    {
        ...
        [TestCategory("UnitTest")]
        [TestCategory("Lambda")]
        public void ListFunctionUrlConfigsTest_TwoPages()
        {
            var request = InstantiateClassGenerator.Execute<ListFunctionUrlConfigsRequest>();

            var firstResponse = InstantiateClassGenerator.Execute<ListFunctionUrlConfigsResponse>();
            var secondResponse = InstantiateClassGenerator.Execute<ListFunctionUrlConfigsResponse>();
            secondResponse.NextMarker = null;

            _mockClient.SetupSequence(x => x.ListFunctionUrlConfigs(request)).Returns(firstResponse).Returns(secondResponse);
            var paginator = _mockClient.Object.Paginators.ListFunctionUrlConfigs(request);
        ...
        [TestCategory("Lambda")]
        [ExpectedException(typeof(System.InvalidOperationException), "Paginator has already been consumed and cannot be reused. Please create a new instance.")]
        public void ListFunctionUrlConfigsTest__OnlyUsedOnce()
        {
            var request = InstantiateClassGenerator.Execute<ListFunctionUrlConfigsRequest>();

            var response = InstantiateClassGenerator.Execute<ListFunctionUrlConfigsResponse>();
            response.NextMarker = null;

            _mockClient.Setup(x => x.ListFunctionUrlConfigs(request)).Returns(response);
            var paginator = _mockClient.Object.Paginators.ListFunctionUrlConfigs(request);
