...
namespace AWSSDK_DotNet35.UnitTests.PaginatorTests
{
    ...
    public class SecurityHubPaginatorTests
    {
        ...
        [TestCategory("UnitTest")]
        [TestCategory("SecurityHub")]
        public void ListFindingAggregatorsTest_TwoPages()
        {
            var request = InstantiateClassGenerator.Execute<ListFindingAggregatorsRequest>();

            var firstResponse = InstantiateClassGenerator.Execute<ListFindingAggregatorsResponse>();
            var secondResponse = InstantiateClassGenerator.Execute<ListFindingAggregatorsResponse>();
            secondResponse.NextToken = null;

            _mockClient.SetupSequence(x => x.ListFindingAggregators(request)).Returns(firstResponse).Returns(secondResponse);
            var paginator = _mockClient.Object.Paginators.ListFindingAggregators(request);
        ...
        [TestCategory("SecurityHub")]
        [ExpectedException(typeof(System.InvalidOperationException), "Paginator has already been consumed and cannot be reused. Please create a new instance.")]
        public void ListFindingAggregatorsTest__OnlyUsedOnce()
        {
            var request = InstantiateClassGenerator.Execute<ListFindingAggregatorsRequest>();

            var response = InstantiateClassGenerator.Execute<ListFindingAggregatorsResponse>();
            response.NextToken = null;

            _mockClient.Setup(x => x.ListFindingAggregators(request)).Returns(response);
            var paginator = _mockClient.Object.Paginators.ListFindingAggregators(request);
