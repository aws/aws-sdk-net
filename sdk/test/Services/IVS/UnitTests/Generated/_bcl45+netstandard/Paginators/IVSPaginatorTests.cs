...
namespace AWSSDK_DotNet35.UnitTests.PaginatorTests
{
    ...
    public class IVSPaginatorTests
    {
        ...
        [TestCategory("UnitTest")]
        [TestCategory("IVS")]
        public void ListStreamSessionsTest_TwoPages()
        {
            var request = InstantiateClassGenerator.Execute<ListStreamSessionsRequest>();

            var firstResponse = InstantiateClassGenerator.Execute<ListStreamSessionsResponse>();
            var secondResponse = InstantiateClassGenerator.Execute<ListStreamSessionsResponse>();
            secondResponse.NextToken = null;

            _mockClient.SetupSequence(x => x.ListStreamSessions(request)).Returns(firstResponse).Returns(secondResponse);
            var paginator = _mockClient.Object.Paginators.ListStreamSessions(request);
        ...
        [TestCategory("IVS")]
        [ExpectedException(typeof(System.InvalidOperationException), "Paginator has already been consumed and cannot be reused. Please create a new instance.")]
        public void ListStreamSessionsTest__OnlyUsedOnce()
        {
            var request = InstantiateClassGenerator.Execute<ListStreamSessionsRequest>();

            var response = InstantiateClassGenerator.Execute<ListStreamSessionsResponse>();
            response.NextToken = null;

            _mockClient.Setup(x => x.ListStreamSessions(request)).Returns(response);
            var paginator = _mockClient.Object.Paginators.ListStreamSessions(request);
