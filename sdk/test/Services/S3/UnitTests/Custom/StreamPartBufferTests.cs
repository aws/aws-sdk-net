    ...
    public class StreamPartBufferTests
    {
        ...
        public void Constructor_WithValidParameters_CreatesBuffer()
        {
            ...
            try
            {
                ...
                Assert.AreEqual(actualLength, partBuffer.ActualDataLength);
        ...
        [TestMethod]
        public void Length_ReturnsActualDataLength()
        {
            // Arrange
            byte[] testBuffer = ArrayPool<byte>.Shared.Rent(2048);
        ...
        [TestMethod]
        public void ActualSize_ReturnsActualDataLength()
        {
            // Arrange
            byte[] testBuffer = ArrayPool<byte>.Shared.Rent(2048);
            int actualLength = 750;
            var partBuffer = new StreamPartBuffer(1, testBuffer, actualLength);

            try
            {
                // Act & Assert
                Assert.AreEqual(actualLength, partBuffer.ActualSize);
            }
            finally
            {
                partBuffer.Dispose();
        ...
        public void Constructor_WithEmptyBuffer_HandlesCorrectly()
        {
            ...
            {
                // Assert
                Assert.AreEqual(0, partBuffer.ActualDataLength);
                Assert.AreEqual(0, partBuffer.RemainingBytes);
                Assert.AreEqual(0, partBuffer.CurrentPosition);
            }
            finally
