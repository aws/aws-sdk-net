...
{
    public partial class DocumentTests : TestBase<AmazonDynamoDBClient>
    {
        [Fact]
        //[Category("DynamoDB")]
        public void TestTableOperations()
        {
            RunAsSync(async () =>
            {
                ...
                {
                    using (var testSession = new DynamoDBTestSession("TestTableOperations", Client))
                    {
                        await testSession.CreateTestTables(50,50);

                        Table hashTable;
                        Table hashRangeTable;

                        // Load tables using provided conversion schema
                        LoadTables(conversion, out hashTable, out hashRangeTable, testSession);
        ...
        {            
            await ClearTable(hashRangeTable.TableName);
        ...
        private void LoadTables(DynamoDBEntryConversion conversion, out Table hashTable, out Table hashRangeTable, DynamoDBTestSession testSession)
        {
            testSession.TableCache.Clear();
            ...
            using (var counter = CountServiceResponses(Client))
            {
                ...
                Assert.True(Table.TryLoadTable(Client, testSession.HashTableName, conversion, out hashTable));
                Assert.Equal(1, counter.ResponseCount);

                Assert.NotNull(hashTable);
                Assert.Equal(testSession.HashTableName, hashTable.TableName);
                ...
                Assert.Equal(1, counter.ResponseCount);
                hashRangeTable = Table.LoadTable(Client, testSession.HashRangeTableName, conversion);
                Assert.Equal(2, counter.ResponseCount);
                Assert.NotNull(hashRangeTable);
                Assert.Equal(testSession.HashRangeTableName, hashRangeTable.TableName);
