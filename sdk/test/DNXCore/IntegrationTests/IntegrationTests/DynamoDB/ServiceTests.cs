...
{
    public partial class DynamoDBServiceTests : TestBase<AmazonDynamoDBClient>
    {
        [Fact]
        //[Category("DynamoDB")]
        public void TestTableCalls()
        {
            RunAsSync(async () =>
            {
                using (var testSession = new DynamoDBTestSession("TestTableCalls", Client))
                {
                    await TestTableCallsAsync(testSession);
                }
...
#pragma warning restore 162
        ...
        private async Task TestTableCallsAsync(DynamoDBTestSession testSession)
        {
            var createdTables = new List<string>();

            // Create hash-key table
            var table1Name = testSession.TableNamePrefix + "Table1";
            await Client.CreateTableAsync(
                ...
                new ProvisionedThroughput { ReadCapacityUnits = DynamoDBTestSession.DefaultReadCapacity,
                    WriteCapacityUnits = DynamoDBTestSession.DefaultWriteCapacity
                });
            createdTables.Add(table1Name);

            // Create hash-and-range-key table
            var table2Name = testSession.TableNamePrefix + "Table2";
            await Client.CreateTableAsync(
                table2Name,
                new List<KeySchemaElement>
                    {
            ...
            var table3Name = testSession.TableNamePrefix + "Table3";
            await Client.CreateTableAsync(new CreateTableRequest
            {
                ...
                ProvisionedThroughput = new ProvisionedThroughput { ReadCapacityUnits = DynamoDBTestSession.DefaultReadCapacity,
                    WriteCapacityUnits = DynamoDBTestSession.DefaultWriteCapacity
                },
            });
            ...
            DynamoDBTestSession.WaitForTableStatus(Client, createdTables, TableStatus.ACTIVE);

            // Create hash-and-range-key table with local and global indexes
            var table4Name = testSession.TableNamePrefix + "Table4";
            await Client.CreateTableAsync(new CreateTableRequest
            {
            ...
            Assert.True(createdTables.All(t => tables.Contains(t)));
        ...
        [Fact]
        public async Task TestDataCallsAsync()
        {
            using (var testSession = new DynamoDBTestSession("TestTableCalls", Client))
            {
                await testSession.CreateTestTables(10,5);

                //await testSession.CreateTestTables(writeCapacity: 2, readCapacity: 5);

                // Test hash-key table
                await TestHashTable(testSession.HashTableName);

                // Test hash-and-range-key table
                await TestHashRangeTable(testSession.HashRangeTableName);

                // Test batch gets and writes
                await TestBatchWriteGet(testSession.HashTableName, testSession.HashRangeTableName);

                // Test large batch gets and writes
                await TestLargeBatches(testSession.HashTableName);
            }
        ...
        private async Task TestLargeBatches(string hashTableName)
        {
            ...
            Assert.True(itemSize < DynamoDBTestSession.MaxItemSize);
            int itemCount = 25;

            // DynamoDB allows 1MB of data per operation, so itemSize * writeBatchSize < 1MB
            int writeBatchSize = (int)Math.Floor((decimal)DynamoDBTestSession.OneMB / (decimal)itemSize);
            ...
            for (int i = 0; i < itemCount; i += DynamoDBTestSession.MaxBatchSize)
            {
                List<string> itemsToGet = itemIds.GetRange(i, DynamoDBTestSession.MaxBatchSize);
        ...
        private async Task<List<Dictionary<string, AttributeValue>>> ScanHelper(string tableName, Dictionary<string, Condition> conditions, int? segment = null, int? totalSegments = null)
        {
            ...
            var request = new ScanRequest
            {
                ...
                Limit = DynamoDBTestSession.ScanLimit,
            };
