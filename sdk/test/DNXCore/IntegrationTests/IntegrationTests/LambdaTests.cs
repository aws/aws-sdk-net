...
namespace Amazon.DNXCore.IntegrationTests
{
    ...
    public class LambdaTests : TestBase<AmazonLambdaClient>
	{
        ...
        public void ListFunctionsTest()
        {
            ...
            Assert.NotNull(result.ResponseMetadata);
            Assert.NotNull(result.ResponseMetadata.RequestId);
        }
...
".Trim();
        ...
        public void LambdaFunctionTest()
        {
            ...
            try
            {
                ...
                Assert.Equal("helloworld.handler", function.Handler);
                Assert.Equal(iamRoleArn, function.Role);
                ...
                Assert.Equal("helloworld.handler", getFunctionResponse.Configuration.Handler);
                Assert.NotNull(getFunctionResponse.Code.Location);
                ...
                Assert.Equal("helloworld.handler", getFunctionConfiguration.Handler);
                ...
                Assert.Equal(invokeAsyncResponse.Status, 202); // Status Code Accepted
                ...
                Assert.Equal(clientContext, request.ClientContext);
                Assert.Equal(clientContextBase64, request.ClientContextBase64);

                // Call the function sync
                var invokeSyncResponse = Client.InvokeAsync(request).Result;
                Assert.Null(invokeSyncResponse.FunctionError);
                Assert.Null(invokeSyncResponse.LogResult);
                Assert.NotNull(invokeSyncResponse.Payload);
                Assert.NotEqual(0, invokeSyncResponse.Payload.Length);
                Assert.NotEqual(0, invokeSyncResponse.StatusCode);
                ...
                Assert.Null(invokeSyncResponse.LogResult);
                Assert.NotNull(invokeSyncResponse.Payload);
                Assert.Equal(0, invokeSyncResponse.Payload.Length);
                Assert.NotEqual(0, invokeSyncResponse.StatusCode);
                ...
                Assert.NotNull(invokeSyncResponse.FunctionError);
                Assert.Null(invokeSyncResponse.LogResult);
                Assert.NotNull(invokeSyncResponse.Payload);
                Assert.NotEqual(0, invokeSyncResponse.Payload.Length);
                Assert.NotEqual(0, invokeSyncResponse.StatusCode);
        ...
        public void CreateLambdaFunction(string functionName, string iamRoleName, out string iamRoleArn, out string functionArn)
        {
            ...
            Assert.True(uploadResponse.CodeSize > 0);
            Assert.NotNull(uploadResponse.FunctionArn);
