...
namespace Amazon.DNXCore.IntegrationTests
{
    ...
    public class CloudTrail : TestBase<AmazonCloudTrailClient>
    {
        ...
        public CloudTrail()
        {
            ...
            var policy = UtilityMethods.GetFileText("IntegrationTests\\CloudTrailBucketPolicy.json");
        ...
        [Fact]
        [Trait(CategoryAttribute,"CloudTrail")]
        public void CloudTrailIntegrationTest()
        {
            ...
            Assert.Equal(trail.Name, trailName);
            Assert.Equal(trail.IncludeGlobalServiceEvents, true);
            Assert.Equal(trail.S3BucketName, _trailStorageBucketName);
            Assert.Equal(trail.S3KeyPrefix, S3_PREFIX);
            Assert.Equal(trail.SnsTopicName, null);
            UtilityMethods.Sleep(TimeSpan.FromSeconds(5));

            var trails = Client.DescribeTrailsAsync().Result.TrailList;
            Assert.Equal(trails.Count, trailsCount + 1);

            var trailStatus = Client.GetTrailStatusAsync(new GetTrailStatusRequest
            {
                Name = trailName
            }).Result;
            Assert.Equal(trailStatus.IsLogging, false);
            ...
            Assert.Equal(trailStatus.IsLogging, true);
            ...
            if (allEvents.Count > 0)
            {
                ...
                Assert.Equal(firstResourceNameKVP.Value, filteredEvents.Count);
            }
