    ...
    public class UntagResourceRequestMarshaller : IMarshaller<IRequest, UntagResourceRequest> , IMarshaller<IRequest,AmazonWebServiceRequest>
    {
        ...
        public IRequest Marshall(UntagResourceRequest publicRequest)
        {
            ...
            if (!publicRequest.IsSetResourceArn())
                throw new AmazonSupplyChainException("Request object does not have required field ResourceArn set");
            request.AddPathResource("{resourceArn}", StringUtils.FromString(publicRequest.ResourceArn));
            if (publicRequest.TagKeys == null)
                throw new AmazonSupplyChainException("Request object does not have required field TagKeys set");
            
            if (publicRequest.IsSetTagKeys())
                request.ParameterCollection.Add("tagKeys", publicRequest.TagKeys);
            request.ResourcePath = "/api/tags/{resourceArn}";
