...
namespace Amazon.Wickr.Internal
{
    /// <summary>
    /// Amazon Wickr endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for Wickr service requests.
    /// Collects values for WickrEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses WickrEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonWickrEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonWickrConfig)requestContext.ClientConfig;
            var result = new WickrEndpointParameters();
