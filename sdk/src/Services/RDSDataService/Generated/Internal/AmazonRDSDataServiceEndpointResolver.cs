...
namespace Amazon.RDSDataService.Internal
{
    /// <summary>
    /// Amazon RDSDataService endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for RDSDataService service requests.
    /// Collects values for RDSDataServiceEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses RDSDataServiceEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonRDSDataServiceEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonRDSDataServiceConfig)requestContext.ClientConfig;
            var result = new RDSDataServiceEndpointParameters();
