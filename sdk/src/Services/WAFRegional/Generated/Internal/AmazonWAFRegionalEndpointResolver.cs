...
namespace Amazon.WAFRegional.Internal
{
    /// <summary>
    /// Amazon WAFRegional endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for WAFRegional service requests.
    /// Collects values for WAFRegionalEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses WAFRegionalEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonWAFRegionalEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonWAFRegionalConfig)requestContext.ClientConfig;
            var result = new WAFRegionalEndpointParameters();
