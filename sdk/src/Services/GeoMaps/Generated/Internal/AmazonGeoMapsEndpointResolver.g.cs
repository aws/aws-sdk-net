...
{
    /// <summary>
    /// Amazon GeoMaps endpoint resolver.
    /// <para />
    /// Custom <see cref="PipelineHandler"/> responsible for resolving endpoint and setting authentication parameters for GeoMaps service requests.
    /// <para />
    /// Collects values for <see cref="GeoMapsEndpointParameters"/> and then tries to resolve endpoint by calling
    /// <c>ResolveEndpoint</c> method on <see cref="GlobalEndpoints.Provider"/> if present, otherwise uses <see cref="AmazonGeoMapsEndpointProvider"/>.
    /// <para />
    /// Responsible for setting authentication and HTTP headers provided by resolved endpoint.
    /// </summary>
    public class AmazonGeoMapsEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonGeoMapsConfig)requestContext.ClientConfig;
            var result = new GeoMapsEndpointParameters();
