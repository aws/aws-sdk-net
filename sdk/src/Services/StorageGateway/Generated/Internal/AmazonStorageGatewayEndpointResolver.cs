...
{
    /// <summary>
    /// Amazon StorageGateway endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for StorageGateway service requests.
    /// Collects values for StorageGatewayEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses StorageGatewayEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonStorageGatewayEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonStorageGatewayConfig)requestContext.ClientConfig;
            var result = new StorageGatewayEndpointParameters();
