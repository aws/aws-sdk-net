...
namespace Amazon.ManagedGrafana.Internal
{
    ...
    /// <para />
    /// Custom <see cref="PipelineHandler"/> responsible for resolving endpoint and setting authentication parameters for ManagedGrafana service requests.
    /// <para />
    /// Collects values for <see cref="ManagedGrafanaEndpointParameters"/> and then tries to resolve endpoint by calling
    /// <c>ResolveEndpoint</c> method on <see cref="GlobalEndpoints.Provider"/> if present, otherwise uses <see cref="AmazonManagedGrafanaEndpointProvider"/>.
    /// <para />
