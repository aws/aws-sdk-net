...
{
    /// <summary>
    /// Container for the parameters to the GenerateAuthRequestCryptogram operation. Generates
    /// an Authorization Request Cryptogram (ARQC) for an EMV chip payment card authorization.
    /// For more information, see <a href="https://docs.aws.amazon.com/payment-cryptography/latest/userguide/data-operations.generateauthrequestcryptogram.html">Generate
    /// auth request cryptogram</a> in the <i>Amazon Web Services Payment Cryptography User
    /// Guide</i>. <para> ARQC generation uses an Issuer Master Key (IMK) for application
    /// cryptograms (TR31_E0_EMV_MKEY_APP_CRYPTOGRAMS) to derive a session key, which is then
    /// used to generate the cryptogram from the provided transaction data (when applicable).
    /// To use this operation, you must first create or import an IMK-AC key by calling <a
    ...
    /// key. </para> <important> <para> This operation is intended for development and testing
    /// scenarios only. It is not recommended to use this operation as a substitute for card-based
    /// cryptogram generation in production payment flows. </para> </important> <para> For
    /// information about valid keys for this operation, see <a href="https://docs.aws.amazon.com/payment-cryptography/latest/userguide/keys-validattributes.html">Understanding
    ...
    /// policies</a>. </para> <para> <b>Related operations:</b> </para> <ul> <li> <para> <a>VerifyAuthRequestCryptogram</a>
    /// </para> </li> </ul>
    /// </summary>
