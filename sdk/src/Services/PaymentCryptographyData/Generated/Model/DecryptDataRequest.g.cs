...
{
    /// <summary>
    /// Container for the parameters to the DecryptData operation. Decrypts ciphertext data
    /// to plaintext using a symmetric (TDES, AES), asymmetric (RSA), or derived (DUKPT or
    /// EMV) encryption key scheme. For more information, see <a href="https://docs.aws.amazon.com/payment-cryptography/latest/userguide/decrypt-data.html">Decrypt
    ...
    /// decryption key as a TR-31 WrappedKeyBlock. This can be used when key material is frequently
    /// rotated, such as during every card transaction, and there is need to avoid importing
    /// short-lived keys into Amazon Web Services Payment Cryptography. To decrypt using dynamic
    /// keys, the <c>keyARN</c> is the Key Encryption Key (KEK) of the TR-31 wrapped decryption
    ...
    /// </para> <para> For symmetric and DUKPT decryption, Amazon Web Services Payment Cryptography
    /// supports <c>TDES</c> and <c>AES</c> algorithms. For EMV decryption, Amazon Web Services
    /// Payment Cryptography supports <c>TDES</c> algorithms. For asymmetric decryption, Amazon
    /// Web Services Payment Cryptography supports <c>RSA</c>. </para> <para> When you use
    /// TDES or TDES DUKPT, the ciphertext data length must be a multiple of 8 bytes. For
    /// AES or AES DUKPT, the ciphertext data length must be a multiple of 16 bytes. For RSA,
    /// it sould be equal to the key size unless padding is enabled. </para> <para> For information
    ...
    public partial class DecryptDataRequest : AmazonPaymentCryptographyDataRequest
    {
        ...
        /// <summary>
        /// Checks to see if the DecryptionAttributes property is set.
        /// </summary>
        internal bool IsSetDecryptionAttributes() => this.DecryptionAttributes != null;
