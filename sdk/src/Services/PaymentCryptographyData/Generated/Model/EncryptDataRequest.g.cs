...
{
    /// <summary>
    /// Container for the parameters to the EncryptData operation. Encrypts plaintext data
    /// to ciphertext using a symmetric (TDES, AES), asymmetric (RSA), or derived (DUKPT or
    /// EMV) encryption key scheme. For more information, see <a href="https://docs.aws.amazon.com/payment-cryptography/latest/userguide/encrypt-data.html">Encrypt
    /// data</a> in the <i>Amazon Web Services Payment Cryptography User Guide</i>. <para>
    ...
    /// </para> <para> For this operation, the key must have <c>KeyModesOfUse</c> set to <c>Encrypt</c>.
    ...
    /// </para> <para> This operation also supports dynamic keys, allowing you to pass a dynamic
    /// encryption key as a TR-31 WrappedKeyBlock. This can be used when key material is frequently
    /// rotated, such as during every card transaction, and there is need to avoid importing
    /// short-lived keys into Amazon Web Services Payment Cryptography. To encrypt using dynamic
    /// keys, the <c>keyARN</c> is the Key Encryption Key (KEK) of the TR-31 wrapped encryption
    ...
    /// </para> <para> For symmetric and DUKPT encryption, Amazon Web Services Payment Cryptography
    /// supports <c>TDES</c> and <c>AES</c> algorithms. For EMV encryption, Amazon Web Services
    /// Payment Cryptography supports <c>TDES</c> algorithms.For asymmetric encryption, Amazon
    /// Web Services Payment Cryptography supports <c>RSA</c>. </para> <para> When you use
    /// TDES or TDES DUKPT, the plaintext data length must be a multiple of 8 bytes. For AES
    /// or AES DUKPT, the plaintext data length must be a multiple of 16 bytes. For RSA, it
    /// sould be equal to the key size unless padding is enabled. </para> <para> To encrypt
    /// using DUKPT, you must already have a BDK (Base Derivation Key) key in your account
    /// with <c>KeyModesOfUse</c> set to <c>DeriveKey</c>, or you can generate a new DUKPT
    /// key by calling <a href="https://docs.aws.amazon.com/payment-cryptography/latest/APIReference/API_CreateKey.html">CreateKey</a>.
    /// To encrypt using EMV, you must already have an IMK (Issuer Master Key) key in your
    /// account with <c>KeyModesOfUse</c> set to <c>DeriveKey</c>. </para> <para> For information
    ...
    /// </para> </li> <li> <para> <a>ReEncryptData</a> </para> </li> </ul>
    /// </summary>
    public partial class EncryptDataRequest : AmazonPaymentCryptographyDataRequest
    {
        ...
        /// <summary>
        /// Checks to see if the EncryptionAttributes property is set.
        /// </summary>
        internal bool IsSetEncryptionAttributes() => this.EncryptionAttributes != null;
