    ...
    public class GetDeploymentRequestMarshaller : IMarshaller<IRequest, GetDeploymentRequest> , IMarshaller<IRequest,AmazonWebServiceRequest>
    {
        ...
        public IRequest Marshall(GetDeploymentRequest publicRequest)
        {
            ...
            if (!publicRequest.IsSetDeploymentId())
                throw new AmazonMainframeModernizationException("Request object does not have required field DeploymentId set");
            request.AddPathResource("{deploymentId}", StringUtils.FromString(publicRequest.DeploymentId));
            request.ResourcePath = "/applications/{applicationId}/deployments/{deploymentId}";
