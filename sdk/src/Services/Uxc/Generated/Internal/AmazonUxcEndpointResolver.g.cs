...
namespace Amazon.Uxc.Internal
{
    /// <summary>
    /// Amazon Uxc endpoint resolver.
    /// <para />
    /// Custom <see cref="PipelineHandler"/> responsible for resolving endpoint and setting authentication parameters for Uxc service requests.
    /// <para />
    /// Collects values for <see cref="UxcEndpointParameters"/> and then tries to resolve endpoint by calling
    /// <c>ResolveEndpoint</c> method on <see cref="GlobalEndpoints.Provider"/> if present, otherwise uses <see cref="AmazonUxcEndpointProvider"/>.
    /// <para />
    /// Responsible for setting authentication and HTTP headers provided by resolved endpoint.
    /// </summary>
    public class AmazonUxcEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonUxcConfig)requestContext.ClientConfig;
            var result = new UxcEndpointParameters();
