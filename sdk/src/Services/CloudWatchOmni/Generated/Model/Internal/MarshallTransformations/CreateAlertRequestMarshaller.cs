...
namespace Amazon.CloudWatchOmni.Model.Internal.MarshallTransformations
{
    ...
    public class CreateAlertRequestMarshaller : IMarshaller<IRequest, CreateAlertRequest> , IMarshaller<IRequest,AmazonWebServiceRequest>
    {
        ...
        public IRequest Marshall(CreateAlertRequest publicRequest)
        {
            ...
            request.ResourcePath = "service/CloudWatchOmniFrontend/operation/CreateAlert";
            ...
            try
            {
                ...
                if (publicRequest.IsSetNotificationRules())
                {
                    context.Writer.WriteTextString("notificationRules");
                    context.Writer.WriteStartArray(publicRequest.NotificationRules.Count);
                    foreach(var publicRequestNotificationRulesListValue in publicRequest.NotificationRules)
                    {
                        context.Writer.WriteStartMap(null);

                        var marshaller = NotificationRuleMarshaller.Instance;
                        marshaller.Marshall(publicRequestNotificationRulesListValue, context);

                        context.Writer.WriteEndMap();
                    }
                    context.Writer.WriteEndArray();
                }
                if (publicRequest.IsSetNotificationsEnabled())
                {
                    context.Writer.WriteTextString("notificationsEnabled");
                    context.Writer.WriteBoolean(publicRequest.NotificationsEnabled.Value);
                }
                if (publicRequest.IsSetProfileId())
                {
                    context.Writer.WriteTextString("profileId");
                    context.Writer.WriteTextString(publicRequest.ProfileId);
                }
