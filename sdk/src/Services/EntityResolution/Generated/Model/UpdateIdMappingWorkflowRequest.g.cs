...
{
    /// <summary>
    /// Container for the parameters to the UpdateIdMappingWorkflow operation. Updates an
    /// existing <c>IdMappingWorkflow</c>. This method is identical to CreateIdMappingWorkflow,
    /// except it uses an HTTP <c>PUT</c> request instead of a <c>POST</c> request, and the
    /// <c>IdMappingWorkflow</c> must already exist for the method to succeed. <important>
    /// <para> Incremental processing is not supported for ID mapping workflows. </para> </important>
    /// </summary>
