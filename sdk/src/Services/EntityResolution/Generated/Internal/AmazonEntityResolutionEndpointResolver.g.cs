...
using Amazon.Util;
using Amazon.EntityResolution.Endpoints;

#pragma warning disable 1591

namespace Amazon.EntityResolution.Internal
{
    /// <summary>
    /// Amazon EntityResolution endpoint resolver.
    /// <para />
    /// Custom <see cref="PipelineHandler"/> responsible for resolving endpoint and setting authentication parameters for EntityResolution service requests.
    /// <para />
    /// Collects values for <see cref="EntityResolutionEndpointParameters"/> and then tries to resolve endpoint by calling
    /// <c>ResolveEndpoint</c> method on <see cref="GlobalEndpoints.Provider"/> if present, otherwise uses <see cref="AmazonEntityResolutionEndpointProvider"/>.
    /// <para />
    /// Responsible for setting authentication and HTTP headers provided by resolved endpoint.
    /// </summary>
    public class AmazonEntityResolutionEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonEntityResolutionConfig)requestContext.ClientConfig;
            var result = new EntityResolutionEndpointParameters();
