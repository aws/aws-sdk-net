...
{
    /// <summary>
    /// Container for the parameters to the DescribeRouteCalculator operation. <important>
    /// <para> This operation is no longer current and may be deprecated in the future. We
    /// recommend you upgrade to the Routes API V2 unless you require Grab data. </para> <ul>
    /// <li> <para> <c>DescribeRouteCalculator</c> is part of a previous Amazon Location Service
    /// Routes API (version 1) which has been superseded by a more intuitive, powerful, and
    /// complete API (version 2). </para> </li> <li> <para> The Routes API version 2 has a
    /// simplified interface that can be used without creating or managing route calculator
    /// resources. </para> </li> <li> <para> If you are using an Amazon Web Services SDK or
    /// the Amazon Web Services CLI, note that the Routes API version 2 is found under <c>geo-routes</c>
    ...
    /// Guide</a>. </para> </li> </ul> </important> <para> Retrieves the route calculator
    /// resource details. </para>
    /// </summary>
