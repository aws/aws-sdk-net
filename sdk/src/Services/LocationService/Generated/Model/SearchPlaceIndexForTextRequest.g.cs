...
{
    /// <summary>
    /// Container for the parameters to the SearchPlaceIndexForText operation. <important>
    /// <para> This operation is no longer current and may be deprecated in the future. We
    /// recommend you upgrade to <a href="/location/latest/APIReference/API_geoplaces_Geocode.html">
    /// <c>Geocode</c> </a> or <a href="/location/latest/APIReference/API_geoplaces_SearchText.html">
    /// <c>SearchText</c> </a> unless you require Grab data. </para> <ul> <li> <para> <c>SearchPlaceIndexForText</c>
    ...
    /// <li> <para> The version 2 <c>Geocode</c> operation gives better results in the address
    /// geocoding use case, while the version 2 <c>SearchText</c> operation gives better results
    /// when searching for businesses and points of interest. </para> </li> <li> <para> If
    /// you are using an Amazon Web Services SDK or the Amazon Web Services CLI, note that
    /// the Places API version 2 is found under <c>geo-places</c> or <c>geo_places</c>, not
    /// under <c>location</c>. </para> </li> <li> <para> Since Grab is not yet fully supported
    /// in Places API version 2, we recommend you continue using API version 1 when using
    /// Grab. </para> </li> </ul> </important> <para> Geocodes free-form text, such as an
    /// address, name, city, or region to allow you to search for Places or points of interest.
    /// </para> <para> Optional parameters let you narrow your search results by bounding
    /// box or country, or bias your search toward a specific position on the globe. </para>
    /// <note> <para> You can search for places near a given position using <c>BiasPosition</c>,
    /// or filter results within a bounding box using <c>FilterBBox</c>. Providing both parameters
    /// simultaneously returns an error. </para> </note> <para> Search results are returned
    /// in order of highest to lowest relevance. </para>
    /// </summary>
    public partial class SearchPlaceIndexForTextRequest : AmazonLocationServiceRequest
    {
        ...
        /// For example, <c>123 Any Street</c>.
        /// </para>
        /// </summary>
