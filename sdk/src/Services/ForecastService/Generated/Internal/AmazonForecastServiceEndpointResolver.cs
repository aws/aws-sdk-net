...
namespace Amazon.ForecastService.Internal
{
    /// <summary>
    /// Amazon ForecastService endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for ForecastService service requests.
    /// Collects values for ForecastServiceEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses ForecastServiceEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonForecastServiceEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonForecastServiceConfig)requestContext.ClientConfig;
            var result = new ForecastServiceEndpointParameters();
