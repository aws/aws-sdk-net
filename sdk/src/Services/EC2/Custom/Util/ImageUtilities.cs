    ...
    public static partial class ImageUtilities
    {
        ...
        private static void ParseAMIDefinitions(StreamReader reader)
        {
            try
            {
                ...
                {
                    if (!jsonDocument.RootElement.TryGetProperty(platformTag, out JsonElement imageDefinitions))
                    {
                    ...
                    for (int d = 0; d < imageDefinitions.GetArrayLength(); d++)
                    {
                        ...
                        var key = def.TryGetProperty(DefinitionKeyTag, out JsonElement keyElement)
                            ? keyElement.GetString()
                            : null;
                        var prefix = def.TryGetProperty(DefinitionPrefixTag, out JsonElement prefixElement)
