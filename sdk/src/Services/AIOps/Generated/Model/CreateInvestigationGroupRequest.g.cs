...
{
    /// <summary>
    /// Container for the parameters to the CreateInvestigationGroup operation. Creates an
    /// <i>investigation group</i> in your account. Creating an investigation group is a one-time
    /// setup task for each Region in your account. It is a necessary task to be able to perform
    /// investigations. <para> Settings in the investigation group help you centrally manage
    /// the common properties of your investigations, such as the following: </para> <ul>
    /// <li> <para> Who can access the investigations </para> </li> <li> <para> Whether investigation
    /// data is encrypted with a customer managed Key Management Service key. </para> </li>
    /// <li> <para> How long investigations and their data are retained by default. </para>
    /// </li> </ul> <para> Currently, you can have one investigation group in each Region
    /// in your account. Each investigation in a Region is a part of the investigation group
    /// in that Region </para> <para> To create an investigation group and set up CloudWatch
    /// investigations, you must be signed in to an IAM principal that has either the <c>AIOpsConsoleAdminPolicy</c>
    /// or the <c>AdministratorAccess</c> IAM policy attached, or to an account that has similar
    /// permissions. </para> <important> <para> You can configure CloudWatch alarms to start
    /// investigations and add events to investigations. If you create your investigation
    /// group with <c>CreateInvestigationGroup</c> and you want to enable alarms to do this,
    /// you must use <c>PutInvestigationGroupPolicy</c> to create a resource policy that grants
    /// this permission to CloudWatch alarms. </para> <para> For more information about configuring
    /// CloudWatch alarms, see <a href="https://docs.aws.amazon.com/AmazonCloudWatch/latest/monitoring/AlarmThatSendsEmail.html">Using
