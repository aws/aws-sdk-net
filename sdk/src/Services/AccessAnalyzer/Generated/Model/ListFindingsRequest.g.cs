...
{
    /// <summary>
    /// Container for the parameters to the ListFindings operation. Retrieves a list of findings
    /// generated by the specified analyzer. ListFindings and ListFindingsV2 both use <c>access-analyzer:ListFindings</c>
    /// in the <c>Action</c> element of an IAM policy statement. You must have permission
    /// to perform the <c>access-analyzer:ListFindings</c> action. <para> To learn about filter
    /// keys that you can use to retrieve a list of findings, see <a href="https://docs.aws.amazon.com/IAM/latest/UserGuide/access-analyzer-reference-filter-keys.html">IAM
    /// Access Analyzer filter keys</a> in the <b>IAM User Guide</b>. </para> <note> <para>
    /// ListFindings is supported only for external access analyzers. You must use ListFindingsV2
    /// for internal and unused access analyzers. </para> </note>
    /// </summary>
