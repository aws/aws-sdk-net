...
{
    /// <summary>
    /// Amazon AppStream endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for AppStream service requests.
    /// Collects values for AppStreamEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses AppStreamEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonAppStreamEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonAppStreamConfig)requestContext.ClientConfig;
            var result = new AppStreamEndpointParameters();
