...
namespace Amazon.CloudWatchLogs.Model.Internal.MarshallTransformations
{
    ...
    public class CreateDeliveryRequestMarshaller : IMarshaller<IRequest, CreateDeliveryRequest> , IMarshaller<IRequest,AmazonWebServiceRequest>
    {
        ...
        public IRequest Marshall(CreateDeliveryRequest publicRequest)
        {
            ...
            request.ResourcePath = "service/Logs_20140328/operation/CreateDelivery";
            ...
            try
            {
                ...
                {
                    context.Writer.WriteTextString("deliveryDestinationArn");
                    context.Writer.WriteTextString(publicRequest.DeliveryDestinationArn);
                }
                if (publicRequest.IsSetDeliverySourceName())
                {
                    context.Writer.WriteTextString("deliverySourceName");
                    context.Writer.WriteTextString(publicRequest.DeliverySourceName);
                }
                if (publicRequest.IsSetFieldDelimiter())
                {
                    context.Writer.WriteTextString("fieldDelimiter");
                    context.Writer.WriteTextString(publicRequest.FieldDelimiter);
                }
                if (publicRequest.IsSetRecordFields())
                {
                    context.Writer.WriteTextString("recordFields");
                    context.Writer.WriteStartArray(publicRequest.RecordFields.Count);
                    foreach(var publicRequestRecordFieldsListValue in publicRequest.RecordFields)
                    {
                            context.Writer.WriteTextString(publicRequestRecordFieldsListValue);
                    }
                    context.Writer.WriteEndArray();
                }
                if (publicRequest.IsSetS3DeliveryConfiguration())
                {
                    context.Writer.WriteTextString("s3DeliveryConfiguration");
