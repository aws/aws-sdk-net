...
{
    /// <summary>
    /// Amazon AppRegistry endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for AppRegistry service requests.
    /// Collects values for AppRegistryEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses AppRegistryEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonAppRegistryEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonAppRegistryConfig)requestContext.ClientConfig;
            var result = new AppRegistryEndpointParameters();
