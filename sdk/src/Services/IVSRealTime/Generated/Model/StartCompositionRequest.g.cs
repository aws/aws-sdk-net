...
{
    /// <summary>
    /// Container for the parameters to the StartComposition operation. Starts a Composition
    /// from a stage based on the configuration provided in the request. <para> A Composition
    /// is an ephemeral resource that exists after this operation returns successfully. Composition
    /// stops and the resource is deleted: </para> <ul> <li> <para> When <a>StopComposition</a>
    /// is called. </para> </li> <li> <para> After a 1-minute timeout, when all participants
    /// are disconnected from the stage. </para> </li> <li> <para> After a 1-minute timeout,
    /// if there are no participants in the stage when StartComposition is called. </para>
    /// </li> <li> <para> When broadcasting to the IVS channel fails and all retries are exhausted.
    /// </para> </li> <li> <para> When broadcasting is disconnected and all attempts to reconnect
    /// are exhausted. </para> </li> </ul>
    /// </summary>
    public partial class StartCompositionRequest : AmazonIVSRealTimeRequest
    {
        ...
        public List<DestinationConfiguration> Destinations { get; set; } = AWSConfigs.InitializeCollections ? new List<DestinationConfiguration>() : null;
