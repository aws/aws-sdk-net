...
{
    /// <summary>
    /// Container for the parameters to the GetIceServerConfig operation. Gets the Interactive
    /// Connectivity Establishment (ICE) server configuration information, including URIs,
    /// username, and password which can be used to configure the WebRTC connection. The ICE
    /// component uses this configuration information to setup the WebRTC connection, including
    /// authenticating with the Traversal Using Relays around NAT (TURN) relay server. <para>
    ...
    /// REST API For Access To TURN Services</a>. </para> <para> You can invoke this API to
    /// establish a fallback mechanism in case either of the peers is unable to establish
    /// a direct peer-to-peer connection over a signaling channel. You must specify either
    /// a signaling channel ARN or the client ID in order to invoke this API. </para>
    /// </summary>
