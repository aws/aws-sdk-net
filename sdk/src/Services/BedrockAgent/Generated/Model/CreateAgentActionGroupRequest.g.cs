...
{
    /// <summary>
    /// Container for the parameters to the CreateAgentActionGroup operation. Creates an action
    /// group for an agent. An action group represents the actions that an agent can carry
    /// out for the customer by defining the APIs that an agent can call and the logic for
    /// calling them. <para> To allow your agent to request the user for additional information
    /// when trying to complete a task, add an action group with the <c>parentActionGroupSignature</c>
    /// field set to <c>AMAZON.UserInput</c>. </para> <para> To allow your agent to generate,
    /// run, and troubleshoot code when trying to complete a task, add an action group with
    /// the <c>parentActionGroupSignature</c> field set to <c>AMAZON.CodeInterpreter</c>.
    /// </para> <para> You must leave the <c>description</c>, <c>apiSchema</c>, and <c>actionGroupExecutor</c>
