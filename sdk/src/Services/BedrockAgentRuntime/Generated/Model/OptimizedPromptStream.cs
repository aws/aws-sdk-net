...
namespace Amazon.BedrockAgentRuntime.Model
{
    ...
    public sealed class OptimizedPromptStream : EnumerableEventOutputStream<IEventStreamEvent, BedrockAgentRuntimeEventStreamException>
    {
        /// <summary>
        ...
        new Dictionary<string,Func<IEventStreamMessage,IEventStreamEvent>>(StringComparer.OrdinalIgnoreCase)
        {
            ...
            {"AnalyzePromptEvent", payload => 
                {
                    var context = EventStreamUtils.ConvertMessageToJsonContext(payload);
                    var reader = new StreamingUtf8JsonReader(context.Stream);
                    return new AnalyzePromptEventUnmarshaller().Unmarshall(context, ref reader);
                }
            },
            {"OptimizedPromptEvent", payload => 
                {
                    var context = EventStreamUtils.ConvertMessageToJsonContext(payload);
                    var reader = new StreamingUtf8JsonReader(context.Stream);
                    return new OptimizedPromptEventUnmarshaller().Unmarshall(context, ref reader);
                }
            },
        };
        /// <summary>
