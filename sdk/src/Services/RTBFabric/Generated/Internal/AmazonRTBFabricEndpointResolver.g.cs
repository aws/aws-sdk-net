...
{
    /// <summary>
    /// Amazon RTBFabric endpoint resolver.
    /// <para />
    /// Custom <see cref="PipelineHandler"/> responsible for resolving endpoint and setting authentication parameters for RTBFabric service requests.
    /// <para />
    /// Collects values for <see cref="RTBFabricEndpointParameters"/> and then tries to resolve endpoint by calling
    /// <c>ResolveEndpoint</c> method on <see cref="GlobalEndpoints.Provider"/> if present, otherwise uses <see cref="AmazonRTBFabricEndpointProvider"/>.
    /// <para />
    /// Responsible for setting authentication and HTTP headers provided by resolved endpoint.
    /// </summary>
    public class AmazonRTBFabricEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonRTBFabricConfig)requestContext.ClientConfig;
            var result = new RTBFabricEndpointParameters();
