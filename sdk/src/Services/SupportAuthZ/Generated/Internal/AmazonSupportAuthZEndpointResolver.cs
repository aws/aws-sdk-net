...
{
    /// <summary>
    /// Amazon SupportAuthZ endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for SupportAuthZ service requests.
    /// Collects values for SupportAuthZEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses SupportAuthZEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonSupportAuthZEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonSupportAuthZConfig)requestContext.ClientConfig;
            var result = new SupportAuthZEndpointParameters();
