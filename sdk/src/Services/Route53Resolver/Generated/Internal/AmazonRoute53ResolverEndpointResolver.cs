...
namespace Amazon.Route53Resolver.Internal
{
    /// <summary>
    /// Amazon Route53Resolver endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for Route53Resolver service requests.
    /// Collects values for Route53ResolverEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses Route53ResolverEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonRoute53ResolverEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonRoute53ResolverConfig)requestContext.ClientConfig;
            var result = new Route53ResolverEndpointParameters();
