...
{
    /// <summary>
    /// Container for the parameters to the CreateStreamKey operation. Creates a stream key,
    /// used to initiate a stream, for the specified channel ARN. <para> Note that <a>CreateChannel</a>
    /// creates a stream key. If you subsequently use CreateStreamKey on the same channel,
    /// it will fail because a stream key already exists and there is a limit of 1 stream
    /// key per channel. To reset the stream key on a channel, use <a>DeleteStreamKey</a>
    /// and then CreateStreamKey. </para>
    /// </summary>
