...
{
    /// <summary>
    /// Container for the parameters to the CreateRecordingConfiguration operation. Creates
    /// a new recording configuration, used to enable recording to Amazon S3. <para> <b>Known
    /// issue:</b> In the us-east-1 region, if you use the Amazon Web Services CLI to create
    /// a recording configuration, it returns success even if the S3 bucket is in a different
    /// region. In this case, the <c>state</c> of the recording configuration is <c>CREATE_FAILED</c>
    /// (instead of <c>ACTIVE</c>). (In other regions, the CLI correctly returns failure if
    /// the bucket is in a different region.) </para> <para> <b>Workaround:</b> Ensure that
    /// your S3 bucket is in the same region as the recording configuration. If you create
    /// a recording configuration in a different region as your S3 bucket, delete that recording
    /// configuration and create a new one with an S3 bucket from the correct region. </para>
    /// </summary>
