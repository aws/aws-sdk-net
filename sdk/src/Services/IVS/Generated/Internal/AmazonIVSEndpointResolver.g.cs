...
{
    /// <summary>
    /// Amazon IVS endpoint resolver.
    /// <para />
    /// Custom <see cref="PipelineHandler"/> responsible for resolving endpoint and setting authentication parameters for IVS service requests.
    /// <para />
    /// Collects values for <see cref="IVSEndpointParameters"/> and then tries to resolve endpoint by calling
    /// <c>ResolveEndpoint</c> method on <see cref="GlobalEndpoints.Provider"/> if present, otherwise uses <see cref="AmazonIVSEndpointProvider"/>.
    /// <para />
    /// Responsible for setting authentication and HTTP headers provided by resolved endpoint.
    /// </summary>
    public class AmazonIVSEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonIVSConfig)requestContext.ClientConfig;
            var result = new IVSEndpointParameters();
