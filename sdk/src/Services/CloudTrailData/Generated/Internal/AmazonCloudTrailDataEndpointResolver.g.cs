...
namespace Amazon.CloudTrailData.Internal
{
    ...
    /// <para />
    /// Custom <see cref="PipelineHandler"/> responsible for resolving endpoint and setting authentication parameters for CloudTrailData service requests.
    /// <para />
    /// Collects values for <see cref="CloudTrailDataEndpointParameters"/> and then tries to resolve endpoint by calling
    /// <c>ResolveEndpoint</c> method on <see cref="GlobalEndpoints.Provider"/> if present, otherwise uses <see cref="AmazonCloudTrailDataEndpointProvider"/>.
    /// <para />
