...
{
    /// <summary>
    /// Amazon MemoryDB endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for MemoryDB service requests.
    /// Collects values for MemoryDBEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses MemoryDBEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonMemoryDBEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonMemoryDBConfig)requestContext.ClientConfig;
            var result = new MemoryDBEndpointParameters();
