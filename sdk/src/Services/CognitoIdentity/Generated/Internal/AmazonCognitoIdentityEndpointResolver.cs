...
{
    /// <summary>
    /// Amazon CognitoIdentity endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for CognitoIdentity service requests.
    /// Collects values for CognitoIdentityEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses CognitoIdentityEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonCognitoIdentityEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonCognitoIdentityConfig)requestContext.ClientConfig;
            var result = new CognitoIdentityEndpointParameters();
