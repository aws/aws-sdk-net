...
{
    /// <summary>
    /// Amazon IoTTwinMaker endpoint resolver.
    /// <para />
    /// Custom <see cref="PipelineHandler"/> responsible for resolving endpoint and setting authentication parameters for IoTTwinMaker service requests.
    /// <para />
    /// Collects values for <see cref="IoTTwinMakerEndpointParameters"/> and then tries to resolve endpoint by calling
    /// <c>ResolveEndpoint</c> method on <see cref="GlobalEndpoints.Provider"/> if present, otherwise uses <see cref="AmazonIoTTwinMakerEndpointProvider"/>.
    /// <para />
    /// Responsible for setting authentication and HTTP headers provided by resolved endpoint.
    /// </summary>
    public class AmazonIoTTwinMakerEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonIoTTwinMakerConfig)requestContext.ClientConfig;
            var result = new IoTTwinMakerEndpointParameters();
