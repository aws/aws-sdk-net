...
namespace Amazon.KinesisVideo.Internal
{
    ...
    /// <para />
    /// Custom <see cref="PipelineHandler"/> responsible for resolving endpoint and setting authentication parameters for KinesisVideo service requests.
    /// <para />
    /// Collects values for <see cref="KinesisVideoEndpointParameters"/> and then tries to resolve endpoint by calling
    /// <c>ResolveEndpoint</c> method on <see cref="GlobalEndpoints.Provider"/> if present, otherwise uses <see cref="AmazonKinesisVideoEndpointProvider"/>.
    /// <para />
    /// Responsible for setting authentication and HTTP headers provided by resolved endpoint.
    /// </summary>
    public class AmazonKinesisVideoEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonKinesisVideoConfig)requestContext.ClientConfig;
            var result = new KinesisVideoEndpointParameters();
