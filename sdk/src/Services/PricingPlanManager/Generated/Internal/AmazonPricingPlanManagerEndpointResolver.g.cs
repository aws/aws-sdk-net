...
namespace Amazon.PricingPlanManager.Internal
{
    ...
    /// <para />
    /// Custom <see cref="PipelineHandler"/> responsible for resolving endpoint and setting authentication parameters for PricingPlanManager service requests.
    /// <para />
    /// Collects values for <see cref="PricingPlanManagerEndpointParameters"/> and then tries to resolve endpoint by calling
    /// <c>ResolveEndpoint</c> method on <see cref="GlobalEndpoints.Provider"/> if present, otherwise uses <see cref="AmazonPricingPlanManagerEndpointProvider"/>.
    /// <para />
