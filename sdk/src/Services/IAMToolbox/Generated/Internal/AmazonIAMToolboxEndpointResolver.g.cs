...
{
    /// <summary>
    /// Amazon IAMToolbox endpoint resolver.
    /// <para />
    /// Custom <see cref="PipelineHandler"/> responsible for resolving endpoint and setting authentication parameters for IAMToolbox service requests.
    /// <para />
    /// Collects values for <see cref="IAMToolboxEndpointParameters"/> and then tries to resolve endpoint by calling
    /// <c>ResolveEndpoint</c> method on <see cref="GlobalEndpoints.Provider"/> if present, otherwise uses <see cref="AmazonIAMToolboxEndpointProvider"/>.
    /// <para />
    /// Responsible for setting authentication and HTTP headers provided by resolved endpoint.
    /// </summary>
    public class AmazonIAMToolboxEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonIAMToolboxConfig)requestContext.ClientConfig;
            var result = new IAMToolboxEndpointParameters();
