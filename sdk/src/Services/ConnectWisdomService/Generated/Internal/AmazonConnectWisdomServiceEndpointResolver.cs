...
{
    /// <summary>
    /// Amazon ConnectWisdomService endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for ConnectWisdomService service requests.
    /// Collects values for ConnectWisdomServiceEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses ConnectWisdomServiceEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonConnectWisdomServiceEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonConnectWisdomServiceConfig)requestContext.ClientConfig;
            var result = new ConnectWisdomServiceEndpointParameters();
