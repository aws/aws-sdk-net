...
using Amazon.Util;
using Amazon.LambdaCore.Endpoints;
...
{
    /// <summary>
    /// Amazon LambdaCore endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for LambdaCore service requests.
    /// Collects values for LambdaCoreEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses LambdaCoreEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonLambdaCoreEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonLambdaCoreConfig)requestContext.ClientConfig;
            var result = new LambdaCoreEndpointParameters();
