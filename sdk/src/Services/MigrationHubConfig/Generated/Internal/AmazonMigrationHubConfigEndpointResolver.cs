...
namespace Amazon.MigrationHubConfig.Internal
{
    /// <summary>
    /// Amazon MigrationHubConfig endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for MigrationHubConfig service requests.
    /// Collects values for MigrationHubConfigEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses MigrationHubConfigEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonMigrationHubConfigEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonMigrationHubConfigConfig)requestContext.ClientConfig;
            var result = new MigrationHubConfigEndpointParameters();
