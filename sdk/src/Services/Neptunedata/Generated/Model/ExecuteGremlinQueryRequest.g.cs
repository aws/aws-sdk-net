...
{
    /// <summary>
    /// Container for the parameters to the ExecuteGremlinQuery operation. This commands executes
    /// a Gremlin query. Amazon Neptune is compatible with Apache TinkerPop3 and Gremlin,
    /// so you can use the Gremlin traversal language to query the graph, as described under
    /// <a href="https://tinkerpop.apache.org/docs/current/reference/#graph">The Graph</a>
    /// in the Apache TinkerPop3 documentation. More details can also be found in <a href="https://docs.aws.amazon.com/neptune/latest/userguide/access-graph-gremlin.html">Accessing
    /// a Neptune graph with Gremlin</a>. <para> When invoking this operation in a Neptune
    /// cluster that has IAM authentication enabled, the IAM user or role making the request
    /// must have a policy attached that enables one of the following IAM actions in that
    /// cluster, depending on the query: </para> <ul> <li> <para> <a href="https://docs.aws.amazon.com/neptune/latest/userguide/iam-dp-actions.html#readdataviaquery">neptune-db:ReadDataViaQuery</a>
    ...
    /// </para> </li> </ul> <para> Note that the <a href="https://docs.aws.amazon.com/neptune/latest/userguide/iam-data-condition-keys.html#iam-neptune-condition-keys">neptune-db:QueryLanguage:Gremlin</a>
