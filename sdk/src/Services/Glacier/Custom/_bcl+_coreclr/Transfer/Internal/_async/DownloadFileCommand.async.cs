...
namespace Amazon.Glacier.Transfer.Internal
{
    internal partial class DownloadFileCommand : IDisposable
    {
        ...
        void processMessageAsync(Message message, string jobId)
        {
        ...
        async Task<string> initiateAsync()
        {
        ...
        internal async Task setupTopicAndQueueAsync()
        {
            ...
            this.topicArn = this.snsClient.CreateTopicAsync(new CreateTopicRequest() { Name = "GlacierDownload-" + guidStr }).Result.TopicArn;
            this.queueUrl = this.sqsClient.CreateQueueAsync(new CreateQueueRequest() { QueueName = "GlacierDownload-" + guidStr }).Result.QueueUrl;
            this.queueArn = this.sqsClient.GetQueueAttributesAsync(new GetQueueAttributesRequest() { QueueUrl = this.queueUrl, AttributeNames = new List<string> { SQSConstants.ATTRIBUTE_QUEUE_ARN } }).Result.Attributes[SQSConstants.ATTRIBUTE_QUEUE_ARN];
