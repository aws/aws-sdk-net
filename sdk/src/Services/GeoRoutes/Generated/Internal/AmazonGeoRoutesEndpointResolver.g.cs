...
{
    /// <summary>
    /// Amazon GeoRoutes endpoint resolver.
    /// <para />
    /// Custom <see cref="PipelineHandler"/> responsible for resolving endpoint and setting authentication parameters for GeoRoutes service requests.
    /// <para />
    /// Collects values for <see cref="GeoRoutesEndpointParameters"/> and then tries to resolve endpoint by calling
    /// <c>ResolveEndpoint</c> method on <see cref="GlobalEndpoints.Provider"/> if present, otherwise uses <see cref="AmazonGeoRoutesEndpointProvider"/>.
    /// <para />
    /// Responsible for setting authentication and HTTP headers provided by resolved endpoint.
    /// </summary>
    public class AmazonGeoRoutesEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonGeoRoutesConfig)requestContext.ClientConfig;
            var result = new GeoRoutesEndpointParameters();
