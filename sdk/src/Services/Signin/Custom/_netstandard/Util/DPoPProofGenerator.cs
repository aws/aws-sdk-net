...
namespace Amazon.Signin.Util
{
    ...
    public static class DPoPProofGenerator
    {
        ...
        private static JsonWebKey CreateJwk(AsymmetricKeyParameter publicKey)
        {
            ...
            var xBytes = point.XCoord.ToBigInteger().ToByteArrayUnsigned();
            var yBytes = point.YCoord.ToBigInteger().ToByteArrayUnsigned();

            return new JsonWebKey
            {
                KeyType = "EC",
                Curve = "P-256",
                X = LoginUtils.Base64UrlEncode(PadTo32Bytes(xBytes)),
                Y = LoginUtils.Base64UrlEncode(PadTo32Bytes(yBytes))
            };
        }
    ...
    internal static class JwtExtensions
    {
        ...
        {
            public const string JwtPayloadJti = "jti";
            public const string JwtPayloadHttpMethod = "htm";
            public const string JwtPayloadHttpUri = "htu";
            public const string JwtPayloadIssuedAt = "iat";
            public const string JwtHeaderType = "typ";
            public const string JwtHeaderAlgorithm = "alg";
            public const string JwtHeaderJsonWebKey = "jwk";
            public const string JsonWebKeyKeyType = "kty";
            public const string JsonWebKeyCurve = "crv";
            public const string JsonWebKeyX = "x";
            public const string JsonWebKeyY = "y";
        }

        public static string ToJson(this JwtPayload jwtPayload)
        {
            if (jwtPayload == null)
            {
                return null;
            ...
            {
                [JsonPropertyNames.JwtPayloadJti] = jwtPayload.Jti,
                [JsonPropertyNames.JwtPayloadHttpMethod] = jwtPayload.HttpMethod,
                [JsonPropertyNames.JwtPayloadHttpUri] = jwtPayload.HttpUri,
                [JsonPropertyNames.JwtPayloadIssuedAt] = jwtPayload.IssuedAt
            };

            return JsonSerializerHelper.Serialize<Dictionary<string, object>>(jsonData, new DictionaryStringObjectJsonSerializerContexts(new JsonSerializerOptions { WriteIndented = false }));
        }

        public static string ToJson(this JwtHeader jwtHeader)
        {
            if (jwtHeader == null)
            {
                return null;
            ...
            var jsonData = new Dictionary<string, object>
            {
                [JsonPropertyNames.JwtHeaderType] = jwtHeader.Type,
                [JsonPropertyNames.JwtHeaderAlgorithm] = jwtHeader.Algorithm,
                [JsonPropertyNames.JwtHeaderJsonWebKey] = new Dictionary<string, string>
                {
                    [JsonPropertyNames.JsonWebKeyKeyType] = jwtHeader.Jwk?.KeyType,
                    [JsonPropertyNames.JsonWebKeyCurve] = jwtHeader.Jwk?.Curve,
                    [JsonPropertyNames.JsonWebKeyX] = jwtHeader.Jwk?.X,
                    [JsonPropertyNames.JsonWebKeyY] = jwtHeader.Jwk?.Y
                }
            };
