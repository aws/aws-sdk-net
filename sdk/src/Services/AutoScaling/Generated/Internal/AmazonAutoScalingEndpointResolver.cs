...
{
    /// <summary>
    /// Amazon AutoScaling endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for AutoScaling service requests.
    /// Collects values for AutoScalingEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses AutoScalingEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonAutoScalingEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonAutoScalingConfig)requestContext.ClientConfig;
            var result = new AutoScalingEndpointParameters();
