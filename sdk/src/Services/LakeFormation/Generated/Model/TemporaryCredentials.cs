...
{
    /// <summary>
    /// A temporary set of credentials for an Lake Formation user. These credentials are scoped
    /// down to only access the raw data sources that the user has access to. 
    /// 
    ///  
    /// <para>
    /// The temporary security credentials consist of an access key and a session token. The
    /// access key consists of an access key ID and a secret key. When the credentials are
    /// created, they are associated with an IAM access control policy that limits what the
    /// user can do when using the credentials.
    /// </para>
    /// </summary>
    public partial class TemporaryCredentials
    {
