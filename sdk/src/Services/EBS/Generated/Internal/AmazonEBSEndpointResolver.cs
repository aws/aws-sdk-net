...
namespace Amazon.EBS.Internal
{
    /// <summary>
    /// Amazon EBS endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for EBS service requests.
    /// Collects values for EBSEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses EBSEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonEBSEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonEBSConfig)requestContext.ClientConfig;
            var result = new EBSEndpointParameters();
