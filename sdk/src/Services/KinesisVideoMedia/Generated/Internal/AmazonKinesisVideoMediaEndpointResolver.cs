...
namespace Amazon.KinesisVideoMedia.Internal
{
    /// <summary>
    /// Amazon KinesisVideoMedia endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for KinesisVideoMedia service requests.
    /// Collects values for KinesisVideoMediaEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses KinesisVideoMediaEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonKinesisVideoMediaEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonKinesisVideoMediaConfig)requestContext.ClientConfig;
            var result = new KinesisVideoMediaEndpointParameters();
