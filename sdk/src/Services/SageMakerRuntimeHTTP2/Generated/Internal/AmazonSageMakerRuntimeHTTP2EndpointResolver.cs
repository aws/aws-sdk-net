...
{
    /// <summary>
    /// Amazon SageMakerRuntimeHTTP2 endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for SageMakerRuntimeHTTP2 service requests.
    /// Collects values for SageMakerRuntimeHTTP2EndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses SageMakerRuntimeHTTP2EndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonSageMakerRuntimeHTTP2EndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonSageMakerRuntimeHTTP2Config)requestContext.ClientConfig;
            var result = new SageMakerRuntimeHTTP2EndpointParameters();
