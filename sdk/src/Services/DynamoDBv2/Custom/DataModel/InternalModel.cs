    ...
    internal class StorageConfig
    {
        ...
        internal void ResolveConstructorArguments()
        {
            ...
            for (int i = 0; i < parameters.Length; i++)
            {
                ...
                // Version, atomic counter, and auto-generated timestamp members are mutated by the SDK after a
                // save/update (the new value is written back onto the instance). That write-back is impossible on
                // an immutable member supplied through the constructor, so reject the combination up front rather
                // than silently leaving the value stale (which would break optimistic locking / counters).
                if (match.IsVersion || match.IsCounter || match.IsAutoGeneratedTimestamp)
                {
                    string kind = match.IsVersion ? "a version" : match.IsCounter ? "an atomic counter" : "an auto-generated timestamp";
                    throw new InvalidOperationException(
                        $"Property '{match.PropertyName}' of type {TargetType.FullName} is {kind} property and cannot be supplied through a constructor parameter. " +
