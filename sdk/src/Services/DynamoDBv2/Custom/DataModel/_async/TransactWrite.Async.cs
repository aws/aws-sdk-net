...
namespace Amazon.DynamoDBv2.DataModel
{
    ...
    public abstract partial class TransactWrite
    {
        ...
        public async Task ExecuteAsync(CancellationToken cancellationToken = default(CancellationToken))
        {
            using (var span = TracerProvider.GetTracer(DynamoDBTelemetry.DynamoDBTracerScope)
                .CreateSpan($"{nameof(TransactWrite)}.{nameof(ExecuteAsync)}", null, Runtime.Telemetry.Tracing.SpanKind.CLIENT))
            {
    ...
    public partial class MultiTableTransactWrite
    {
        ...
        {
            using (var span = TracerProvider.GetTracer(DynamoDBTelemetry.DynamoDBTracerScope)
                .CreateSpan($"{nameof(MultiTableTransactWrite)}.{nameof(ExecuteAsync)}", null, Runtime.Telemetry.Tracing.SpanKind.CLIENT))
            {
                await ExecuteHelperAsync(cancellationToken).ConfigureAwait(false);
            }
        }
