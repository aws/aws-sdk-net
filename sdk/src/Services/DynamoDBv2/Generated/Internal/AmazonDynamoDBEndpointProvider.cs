...
namespace Amazon.DynamoDBv2.Internal
{
    ...
    public class AmazonDynamoDBEndpointProvider : IEndpointProvider
    {
        ...
        public Endpoint ResolveEndpoint(EndpointParameters parameters)
        {
            ...
            if (IsSet(refs["Endpoint"]) && IsSet(refs["Region"]) && (refs["PartitionResult"] = Partition((string)refs["Region"])) != null)
            {
                ...
                if (Equals(refs["Endpoint"], Interpolate(@"https://dynamodb.{Region}.{PartitionResult#dualStackDnsSuffix}", refs)))
                {
            ...
            if (IsSet(refs["Region"]))
            {
                if ((refs["PartitionResult"] = Partition((string)refs["Region"])) != null)
                {
                    if (Equals(refs["Region"], "local"))
                    {
                        ...
                        return new Endpoint("http://localhost:8000", InterpolateJson(@"{""authSchemes"":[{""name"":""sigv4"",""signingRegion"":""us-east-1"",""signingName"":""dynamodb""}]}", refs), InterpolateJson(@"", refs));
                    }
