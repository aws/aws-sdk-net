...
namespace Amazon.WAFV2.Model.Internal.MarshallTransformations
{
    ...
    public class OrStatementMarshaller : IRequestMarshaller<OrStatement, JsonMarshallerContext> 
    {
        ...
        /// <returns></returns>
        public void Marshall(OrStatement requestObject, JsonMarshallerContext context)
        {
            if(requestObject == null)
                return;
            if(requestObject.IsSetStatements())
            {
                context.Writer.WritePropertyName("Statements");
                context.Writer.WriteStartArray();
                foreach(var requestObjectStatementsListValue in requestObject.Statements)
                {
                    context.Writer.WriteStartObject();

                    var marshaller = StatementMarshaller.Instance;
                    marshaller.Marshall(requestObjectStatementsListValue, context);
