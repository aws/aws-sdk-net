...
{
    /// <summary>
    /// Amazon CostExplorer endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for CostExplorer service requests.
    /// Collects values for CostExplorerEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses CostExplorerEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonCostExplorerEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonCostExplorerConfig)requestContext.ClientConfig;
            var result = new CostExplorerEndpointParameters();
