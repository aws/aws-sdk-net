...
{
    /// <summary>
    /// Container for the parameters to the GetRecommendations operation. Returns a list of
    /// recommended items. For campaigns, the campaign's Amazon Resource Name (ARN) is required
    /// and the required user and item input depends on the recipe type used to create the
    /// solution backing the campaign as follows: <ul> <li> <para> USER_PERSONALIZATION -
    /// <c>userId</c> required, <c>itemId</c> not used </para> </li> <li> <para> RELATED_ITEMS
    /// - <c>itemId</c> required, <c>userId</c> not used </para> </li> </ul> <note> <para>
    /// Campaigns that are backed by a solution created using a recipe of type PERSONALIZED_RANKING
    /// use the API. </para> </note> <para> For recommenders, the recommender's ARN is required
    /// and the required item and user input depends on the use case (domain-based recipe)
    /// backing the recommender. For information on use case requirements see <a href="https://docs.aws.amazon.com/personalize/latest/dg/domain-use-cases.html">Choosing
    ...
    public partial class GetRecommendationsRequest : AmazonPersonalizeRuntimeRequest
    {
        ...
        /// <summary>
        /// Checks to see if the NumResults property is set.
        /// </summary>
        internal bool IsSetNumResults() => this.NumResults.HasValue;
        ...
        /// </summary>
        [AWSProperty(Min = 0, Max = 1)]
        public List<Promotion> Promotions { get; set; } = AWSConfigs.InitializeCollections ? new List<Promotion>() : null;

        /// <summary>
        /// Checks to see if the Promotions property is set.
        /// </summary>
        internal bool IsSetPromotions() => this.Promotions != null && (this.Promotions.Count > 0 || !AWSConfigs.InitializeCollections);
        ...
        /// <summary>
        /// Checks to see if the RecommenderArn property is set.
        /// </summary>
        internal bool IsSetRecommenderArn() => this.RecommenderArn != null;
