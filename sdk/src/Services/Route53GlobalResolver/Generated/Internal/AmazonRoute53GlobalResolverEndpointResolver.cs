...
{
    /// <summary>
    /// Amazon Route53GlobalResolver endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for Route53GlobalResolver service requests.
    /// Collects values for Route53GlobalResolverEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses Route53GlobalResolverEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonRoute53GlobalResolverEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonRoute53GlobalResolverConfig)requestContext.ClientConfig;
            var result = new Route53GlobalResolverEndpointParameters();
