...
using Amazon.Util;
using Amazon.NovaAct.Endpoints;
...
{
    /// <summary>
    /// Amazon NovaAct endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for NovaAct service requests.
    /// Collects values for NovaActEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses NovaActEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonNovaActEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonNovaActConfig)requestContext.ClientConfig;
            var result = new NovaActEndpointParameters();
