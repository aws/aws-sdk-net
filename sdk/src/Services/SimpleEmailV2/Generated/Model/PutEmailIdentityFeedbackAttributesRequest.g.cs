...
namespace Amazon.SimpleEmailV2.Model
{
    ...
    /// complaint event. <para> If the value is <c>true</c>, you receive email notifications
    /// when bounce or complaint events occur. These notifications are sent to the address
    /// that you specified in the <c>Return-Path</c> header of the original email. </para>
    /// <para> You're required to have a method of tracking bounces and complaints. If you
    /// haven't set up another mechanism for receiving bounce or complaint notifications (for
    /// example, by setting up an event destination), you receive an email notification when
    /// these events occur (even if this setting is disabled). </para>
    /// </summary>
    public partial class PutEmailIdentityFeedbackAttributesRequest : AmazonSimpleEmailServiceV2Request
    {
        ...
        /// <summary>
        /// Checks to see if the EmailForwardingEnabled property is set.
        /// </summary>
        internal bool IsSetEmailForwardingEnabled() => this.EmailForwardingEnabled.HasValue;
