...
namespace Amazon.SimpleEmailV2.Model
{
    ...
    /// to enable, disable, or reinstate sending for the entity. <para> The customer-managed
    /// status works in conjunction with the Amazon Web Services Amazon SES-managed status
    /// to determine the overall sending capability. When you update the customer-managed
    /// status, the Amazon Web Services Amazon SES-managed status remains unchanged. If Amazon
    /// Web Services Amazon SES has disabled the entity, it will not be allowed to send regardless
    /// of the customer-managed status setting. When you reinstate an entity through the customer-managed
    /// status, it can continue sending only if the Amazon Web Services Amazon SES-managed
    /// status also permits sending, even if there are active reputation findings, until the
    /// findings are resolved or new violations occur. </para>
    /// </summary>
