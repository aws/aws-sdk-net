...
{
    /// <summary>
    /// Amazon CloudWatchEvents endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for CloudWatchEvents service requests.
    /// Collects values for CloudWatchEventsEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses CloudWatchEventsEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonCloudWatchEventsEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonCloudWatchEventsConfig)requestContext.ClientConfig;
            var result = new CloudWatchEventsEndpointParameters();
