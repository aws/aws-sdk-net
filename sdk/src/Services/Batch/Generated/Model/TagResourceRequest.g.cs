...
{
    /// <summary>
    /// Container for the parameters to the TagResource operation. Associates the specified
    /// tags to a resource with the specified <c>resourceArn</c>. If existing tags on a resource
    /// aren't specified in the request parameters, they aren't changed. When a resource is
    /// deleted, the tags that are associated with that resource are deleted as well. Batch
    /// resources that support tags are compute environments, jobs, job definitions, job queues,
    /// and scheduling policies. ARNs for child jobs of array and multi-node parallel (MNP)
