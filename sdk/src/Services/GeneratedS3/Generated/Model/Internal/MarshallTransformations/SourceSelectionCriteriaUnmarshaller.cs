...
namespace Amazon.S3.Model.Internal.MarshallTransformations
{
    ...
    public class SourceSelectionCriteriaUnmarshaller : IXmlUnmarshaller<SourceSelectionCriteria, XmlUnmarshallerContext>
    {
        ...
        {
            SourceSelectionCriteria unmarshalledObject = new SourceSelectionCriteria();
            ...
            while (context.Read())
            {
                if (context.IsStartElement || context.IsAttribute)
                {
                    ...
                    {
                        var unmarshaller = ReplicaModificationsUnmarshaller.Instance;
                        unmarshalledObject.ReplicaModifications = unmarshaller.Unmarshall(context);
                        continue;
                    }
                    if (context.TestExpression("SseKmsEncryptedObjects", targetDepth))
                    {
                        var unmarshaller = SseKmsEncryptedObjectsUnmarshaller.Instance;
                        unmarshalledObject.SseKmsEncryptedObjects = unmarshaller.Unmarshall(context);
                        continue;
                    }
                }
