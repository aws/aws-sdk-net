...
namespace Amazon.SageMakerMetrics.Internal
{
    ...
    /// <para />
    /// Custom <see cref="PipelineHandler"/> responsible for resolving endpoint and setting authentication parameters for SageMakerMetrics service requests.
    /// <para />
    /// Collects values for <see cref="SageMakerMetricsEndpointParameters"/> and then tries to resolve endpoint by calling
    /// <c>ResolveEndpoint</c> method on <see cref="GlobalEndpoints.Provider"/> if present, otherwise uses <see cref="AmazonSageMakerMetricsEndpointProvider"/>.
    /// <para />
