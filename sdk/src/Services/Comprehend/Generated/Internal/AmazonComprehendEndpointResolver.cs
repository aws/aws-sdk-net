...
{
    /// <summary>
    /// Amazon Comprehend endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for Comprehend service requests.
    /// Collects values for ComprehendEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses ComprehendEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonComprehendEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonComprehendConfig)requestContext.ClientConfig;
            var result = new ComprehendEndpointParameters();
