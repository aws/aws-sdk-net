...
namespace Amazon.EKSAuth.Internal
{
    ...
    /// <para />
    /// Custom <see cref="PipelineHandler"/> responsible for resolving endpoint and setting authentication parameters for EKSAuth service requests.
    /// <para />
    /// Collects values for <see cref="EKSAuthEndpointParameters"/> and then tries to resolve endpoint by calling
    /// <c>ResolveEndpoint</c> method on <see cref="GlobalEndpoints.Provider"/> if present, otherwise uses <see cref="AmazonEKSAuthEndpointProvider"/>.
    /// <para />
