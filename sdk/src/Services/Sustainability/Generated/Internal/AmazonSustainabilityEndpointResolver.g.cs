...
{
    /// <summary>
    /// Amazon Sustainability endpoint resolver.
    /// <para />
    /// Custom <see cref="PipelineHandler"/> responsible for resolving endpoint and setting authentication parameters for Sustainability service requests.
    /// <para />
    /// Collects values for <see cref="SustainabilityEndpointParameters"/> and then tries to resolve endpoint by calling
    /// <c>ResolveEndpoint</c> method on <see cref="GlobalEndpoints.Provider"/> if present, otherwise uses <see cref="AmazonSustainabilityEndpointProvider"/>.
    /// <para />
    /// Responsible for setting authentication and HTTP headers provided by resolved endpoint.
    /// </summary>
    public class AmazonSustainabilityEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonSustainabilityConfig)requestContext.ClientConfig;
            var result = new SustainabilityEndpointParameters();
