...
{
    /// <summary>
    /// Amazon IoTThingsGraph endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for IoTThingsGraph service requests.
    /// Collects values for IoTThingsGraphEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses IoTThingsGraphEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonIoTThingsGraphEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonIoTThingsGraphConfig)requestContext.ClientConfig;
            var result = new IoTThingsGraphEndpointParameters();
