        ...
        public IRequest Marshall(CreateJobRequest publicRequest)
        {
            ...
            {   
                ...
                if (publicRequest.IsSetOperation())
                {
                    ...
                    if (publicRequest.Operation.IsSetS3UpdateObjectEncryption())
                    {
                        xmlWriter.WriteStartElement("S3UpdateObjectEncryption");
                        if (publicRequest.Operation.S3UpdateObjectEncryption.IsSetObjectEncryption())
                        {
                            xmlWriter.WriteStartElement("ObjectEncryption");
                            if (publicRequest.Operation.S3UpdateObjectEncryption.ObjectEncryption.IsSetSSEKMS())
                            {
                                xmlWriter.WriteStartElement("SSE-KMS");
                                if(publicRequest.Operation.S3UpdateObjectEncryption.ObjectEncryption.SSEKMS.IsSetBucketKeyEnabled())
                                    xmlWriter.WriteElementString("BucketKeyEnabled", StringUtils.FromBool(publicRequest.Operation.S3UpdateObjectEncryption.ObjectEncryption.SSEKMS.BucketKeyEnabled.Value));
