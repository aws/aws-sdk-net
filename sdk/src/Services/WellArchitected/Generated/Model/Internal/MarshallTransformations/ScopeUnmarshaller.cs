...
namespace Amazon.WellArchitected.Model.Internal.MarshallTransformations
{
    ...
    public class ScopeUnmarshaller : IJsonUnmarshaller<Scope, JsonUnmarshallerContext>
    {
        ...
        public Scope Unmarshall(JsonUnmarshallerContext context, ref StreamingUtf8JsonReader reader)
        {
            ...
            {
                if (context.TestExpression("goalIds", targetDepth, ref reader))
                {
                    var unmarshaller = new JsonListUnmarshaller<string, StringUnmarshaller>(StringUnmarshaller.Instance);
                    unmarshalledObject.GoalIds = unmarshaller.Unmarshall(context, ref reader);
                    continue;
                }
                if (context.TestExpression("items", targetDepth, ref reader))
                {
                    var unmarshaller = new JsonListUnmarshaller<PillarItem, PillarItemUnmarshaller>(PillarItemUnmarshaller.Instance);
