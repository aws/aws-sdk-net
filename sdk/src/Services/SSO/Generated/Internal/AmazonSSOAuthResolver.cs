...
namespace Amazon.SSO.Internal
{
    ...
    public class AmazonSSOAuthSchemeParameters : IAuthSchemeParameters
    {
        /// <inheritdoc />
    ...
    /// </summary>
    public class AmazonSSOAuthSchemeHandler : BaseAuthResolverHandler
    {
        /// <summary>
        /// Modeled auth scheme resolver for SSO.
        /// </summary>
        public AmazonSSOAuthSchemeResolver AuthSchemeResolver { get; } = new();

        /// <inheritdoc/>
        protected override List<IAuthSchemeOption> ResolveAuthOptions(IExecutionContext executionContext)
        {
            var requestContext = executionContext.RequestContext;
            var mappedParameters = new AmazonSSOAuthSchemeParameters
            {
    ...
    public class AmazonSSOAuthSchemeResolver : IAuthSchemeResolver<AmazonSSOAuthSchemeParameters>
    {
        /// <inheritdoc />
        public List<IAuthSchemeOption> ResolveAuthScheme(AmazonSSOAuthSchemeParameters authParameters)
        {
            switch (authParameters.Operation)
            {
                case "GetRoleCredentialsRequest":
                    return AuthSchemeOption.DEFAULT_NOAUTH;
                case "ListAccountRolesRequest":
                    return AuthSchemeOption.DEFAULT_NOAUTH;
