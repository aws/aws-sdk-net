...
{
    /// <summary>
    /// Amazon KafkaConnect endpoint resolver.
    /// <para />
    /// Custom <see cref="PipelineHandler"/> responsible for resolving endpoint and setting authentication parameters for KafkaConnect service requests.
    /// <para />
    /// Collects values for <see cref="KafkaConnectEndpointParameters"/> and then tries to resolve endpoint by calling
    /// <c>ResolveEndpoint</c> method on <see cref="GlobalEndpoints.Provider"/> if present, otherwise uses <see cref="AmazonKafkaConnectEndpointProvider"/>.
    /// <para />
    /// Responsible for setting authentication and HTTP headers provided by resolved endpoint.
    /// </summary>
    public class AmazonKafkaConnectEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonKafkaConnectConfig)requestContext.ClientConfig;
            var result = new KafkaConnectEndpointParameters();
