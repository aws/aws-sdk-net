...
{
    /// <summary>
    /// Amazon Inspector endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for Inspector service requests.
    /// Collects values for InspectorEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses InspectorEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonInspectorEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonInspectorConfig)requestContext.ClientConfig;
            var result = new InspectorEndpointParameters();
