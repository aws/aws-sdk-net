...
{
    /// <summary>
    /// Amazon Resiliencehubv2 endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for Resiliencehubv2 service requests.
    /// Collects values for Resiliencehubv2EndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses Resiliencehubv2EndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonResiliencehubv2EndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonResiliencehubv2Config)requestContext.ClientConfig;
            var result = new Resiliencehubv2EndpointParameters();
