...
{
    /// <summary>
    /// Amazon ConnectHealth endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for ConnectHealth service requests.
    /// Collects values for ConnectHealthEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses ConnectHealthEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonConnectHealthEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonConnectHealthConfig)requestContext.ClientConfig;
            var result = new ConnectHealthEndpointParameters();
