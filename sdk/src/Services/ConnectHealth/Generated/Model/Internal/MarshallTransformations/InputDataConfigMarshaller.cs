    ...
    public class InputDataConfigMarshaller : IRequestMarshaller<InputDataConfig, JsonMarshallerContext> 
    {
        ...
        public void Marshall(InputDataConfig requestObject, JsonMarshallerContext context)
        {
            if(requestObject == null)
                return;
            if(requestObject.IsSetFhirServer())
            {
                context.Writer.WritePropertyName("fhirServer");
                context.Writer.WriteStartObject();

                var marshaller = FHIRServerMarshaller.Instance;
                marshaller.Marshall(requestObject.FhirServer, context);

                context.Writer.WriteEndObject();
            }

            if(requestObject.IsSetS3Sources())
            {
                context.Writer.WritePropertyName("s3Sources");
                context.Writer.WriteStartArray();
                foreach(var requestObjectS3SourcesListValue in requestObject.S3Sources)
                {
                    context.Writer.WriteStartObject();

                    var marshaller = S3SourceMarshaller.Instance;
                    marshaller.Marshall(requestObjectS3SourcesListValue, context);
