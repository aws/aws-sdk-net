    ...
    public class GetServiceRequestMarshaller : IMarshaller<IRequest, GetServiceRequest> , IMarshaller<IRequest,AmazonWebServiceRequest>
    {
        ...
        public IRequest Marshall(GetServiceRequest publicRequest)
        {
            ...
            if (!publicRequest.IsSetServiceId())
                throw new AmazonDevOpsAgentException("Request object does not have required field ServiceId set");
            request.AddPathResource("{serviceId}", StringUtils.FromString(publicRequest.ServiceId));
            request.ResourcePath = "/v1/services/{serviceId}";
