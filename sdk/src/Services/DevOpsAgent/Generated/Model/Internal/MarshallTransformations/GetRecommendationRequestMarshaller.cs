    ...
    public class GetRecommendationRequestMarshaller : IMarshaller<IRequest, GetRecommendationRequest> , IMarshaller<IRequest,AmazonWebServiceRequest>
    {
        ...
        public IRequest Marshall(GetRecommendationRequest publicRequest)
        {
            ...
            if (!publicRequest.IsSetRecommendationId())
                throw new AmazonDevOpsAgentException("Request object does not have required field RecommendationId set");
            request.AddPathResource("{recommendationId}", StringUtils.FromString(publicRequest.RecommendationId));
            
            if (publicRequest.IsSetRecommendationVersion())
                request.Parameters.Add("recommendationVersion", StringUtils.FromLong(publicRequest.RecommendationVersion));
            request.ResourcePath = "/backlog/agent-space/{agentSpaceId}/recommendations/{recommendationId}";
