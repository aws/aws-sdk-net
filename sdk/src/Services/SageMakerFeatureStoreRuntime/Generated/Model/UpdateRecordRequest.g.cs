...
{
    /// <summary>
    /// Container for the parameters to the UpdateRecord operation. Updates one or more feature
    /// values for an existing record in the specified feature group. Features that you do
    /// not include in the request remain unchanged. You can update up to 100 features per
    /// call. <important> <para> This operation is available only for feature groups that
    /// use the <c>Standard_V2</c> or <c>InMemory</c> online store type. </para> </important>
    /// <para> The record must already exist. If the record does not exist or has been soft-deleted,
    /// the operation returns a <c>ResourceNotFound</c> error. To create a record, use <c>PutRecord</c>.
    /// </para> <para> If you provide an <c>EventTime</c> that is older than the record's
    /// current <c>EventTime</c>, the service rejects the update with a <c>ConflictException</c>.
    /// If the <c>EventTime</c> is equal to or newer than the current value, the service applies
    /// the update. If you omit <c>EventTime</c>, the service keeps the record's existing
    /// <c>EventTime</c> and applies the update. </para> <para> If you specify a <c>TtlDuration</c>,
    /// you must also provide an <c>EventTime</c> in the request. Otherwise, the operation
    /// returns a <c>ValidationError</c>. </para>
    /// </summary>
    public partial class UpdateRecordRequest : AmazonSageMakerFeatureStoreRuntimeRequest
    {
        ...
        public List<FeatureValue> Features { get; set; } = AWSConfigs.InitializeCollections ? new List<FeatureValue>() : null;
