...
{
    /// <summary>
    /// Container for the parameters to the PutRecord operation. The <c>PutRecord</c> API
    /// is used to ingest a list of <c>Records</c> into your feature group. <para> If a new
    /// record’s <c>EventTime</c> is greater, the new record is written to both the <c>OnlineStore</c>
    /// and <c>OfflineStore</c>. Otherwise, the record is a historic record and it is written
    /// only to the <c>OfflineStore</c>. </para> <para> You can specify the ingestion to be
    /// applied to the <c>OnlineStore</c>, <c>OfflineStore</c>, or both by using the <c>TargetStores</c>
    /// request parameter. </para> <para> You can set the ingested record to expire at a given
    /// time to live (TTL) duration after the record’s event time, <c>ExpiresAt</c> = <c>EventTime</c>
    /// + <c>TtlDuration</c>, by specifying the <c>TtlDuration</c> parameter. A record level
    /// <c>TtlDuration</c> is set when specifying the <c>TtlDuration</c> parameter using the
    /// <c>PutRecord</c> API call. If the input <c>TtlDuration</c> is <c>null</c> or unspecified,
    /// <c>TtlDuration</c> is set to the default feature group level <c>TtlDuration</c>. A
    /// record level <c>TtlDuration</c> supersedes the group level <c>TtlDuration</c>. </para>
    /// </summary>
