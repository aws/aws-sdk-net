...
{
    /// <summary>
    /// Container for the parameters to the BatchWriteRecord operation. Writes a batch of
    /// <c>Records</c> to one or more <c>FeatureGroup</c>s. Use this API for bulk ingestion
    /// of records into the <c>OnlineStore</c> and <c>OfflineStore</c>. <para> You can set
    /// the ingested records to expire at a given time to live (TTL) duration after the record's
    /// event time by specifying the <c>TtlDuration</c> parameter. A request level <c>TtlDuration</c>
    /// applies to all entries that do not specify their own <c>TtlDuration</c>. </para>
    /// </summary>
    public partial class BatchWriteRecordRequest : AmazonSageMakerFeatureStoreRuntimeRequest
    {
        ...
        public List<BatchWriteRecordEntry> Entries { get; set; } = AWSConfigs.InitializeCollections ? new List<BatchWriteRecordEntry>() : null;
