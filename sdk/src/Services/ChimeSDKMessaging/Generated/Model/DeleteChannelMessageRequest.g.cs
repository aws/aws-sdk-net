...
{
    /// <summary>
    /// Container for the parameters to the DeleteChannelMessage operation. Deletes a channel
    /// message. Only admins can perform this action. Deletion makes messages inaccessible
    /// immediately. A background process deletes any revisions created by <c>UpdateChannelMessage</c>.
    /// <note> <para> The <c>x-amz-chime-bearer</c> request header is mandatory. Use the ARN
    /// of the <c>AppInstanceUser</c> or <c>AppInstanceBot</c> that makes the API call as
    /// the value in the header. </para> </note>
    /// </summary>
