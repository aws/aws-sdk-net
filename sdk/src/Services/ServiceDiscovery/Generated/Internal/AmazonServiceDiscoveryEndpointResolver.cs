...
{
    /// <summary>
    /// Amazon ServiceDiscovery endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for ServiceDiscovery service requests.
    /// Collects values for ServiceDiscoveryEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses ServiceDiscoveryEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonServiceDiscoveryEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonServiceDiscoveryConfig)requestContext.ClientConfig;
            var result = new ServiceDiscoveryEndpointParameters();
