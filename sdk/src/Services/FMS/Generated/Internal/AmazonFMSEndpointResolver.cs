...
namespace Amazon.FMS.Internal
{
    /// <summary>
    /// Amazon FMS endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for FMS service requests.
    /// Collects values for FMSEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses FMSEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonFMSEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonFMSConfig)requestContext.ClientConfig;
            var result = new FMSEndpointParameters();
