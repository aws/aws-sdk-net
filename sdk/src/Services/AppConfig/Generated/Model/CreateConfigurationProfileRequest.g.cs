...
{
    /// <summary>
    /// Container for the parameters to the CreateConfigurationProfile operation. Creates
    /// a configuration profile, which is information that enables AppConfig to access the
    /// configuration source. Valid configuration sources include the following: <ul> <li>
    /// <para> Configuration data in YAML, JSON, and other formats stored in the AppConfig
    /// hosted configuration store </para> </li> <li> <para> Configuration data stored as
    /// objects in an Amazon Simple Storage Service (Amazon S3) bucket </para> </li> <li>
    /// <para> Pipelines stored in CodePipeline </para> </li> <li> <para> Secrets stored in
    /// Secrets Manager </para> </li> <li> <para> Standard and secure string parameters stored
    /// in Amazon Web Services Systems Manager Parameter Store </para> </li> <li> <para> Configuration
    /// data in SSM documents stored in the Systems Manager document store </para> </li> </ul>
    /// <para> A configuration profile includes the following information: </para> <ul> <li>
    /// <para> The URI location of the configuration data. </para> </li> <li> <para> The Identity
    /// and Access Management (IAM) role that provides access to the configuration data. </para>
    /// </li> <li> <para> A validator for the configuration data. Available validators include
    /// either a JSON Schema or an Amazon Web Services Lambda function. </para> </li> </ul>
    /// <para> For more information, see <a href="http://docs.aws.amazon.com/appconfig/latest/userguide/appconfig-creating-configuration-and-profile.html">Create
