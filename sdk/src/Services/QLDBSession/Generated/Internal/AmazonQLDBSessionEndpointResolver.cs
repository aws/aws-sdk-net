...
namespace Amazon.QLDBSession.Internal
{
    /// <summary>
    /// Amazon QLDBSession endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for QLDBSession service requests.
    /// Collects values for QLDBSessionEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses QLDBSessionEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonQLDBSessionEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonQLDBSessionConfig)requestContext.ClientConfig;
            var result = new QLDBSessionEndpointParameters();
