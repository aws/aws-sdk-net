...
{
    /// <summary>
    /// Amazon KinesisFirehose endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for KinesisFirehose service requests.
    /// Collects values for KinesisFirehoseEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses KinesisFirehoseEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonKinesisFirehoseEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonKinesisFirehoseConfig)requestContext.ClientConfig;
            var result = new KinesisFirehoseEndpointParameters();
