    ...
    public class GetServiceRequestMarshaller : IMarshaller<IRequest, GetServiceRequest> , IMarshaller<IRequest,AmazonWebServiceRequest>
    {
        ...
        public IRequest Marshall(GetServiceRequest publicRequest)
        {
            ...
            if (!publicRequest.IsSetServiceIdentifier())
                throw new AmazonMigrationHubRefactorSpacesException("Request object does not have required field ServiceIdentifier set");
            request.AddPathResource("{ServiceIdentifier}", StringUtils.FromString(publicRequest.ServiceIdentifier));
            request.ResourcePath = "/environments/{EnvironmentIdentifier}/applications/{ApplicationIdentifier}/services/{ServiceIdentifier}";
