...
using Amazon.Util;
using Amazon.Chatbot.Endpoints;

#pragma warning disable 1591

namespace Amazon.Chatbot.Internal
{
    /// <summary>
    /// Amazon Chatbot endpoint resolver.
    /// <para />
    /// Custom <see cref="PipelineHandler"/> responsible for resolving endpoint and setting authentication parameters for Chatbot service requests.
    /// <para />
    /// Collects values for <see cref="ChatbotEndpointParameters"/> and then tries to resolve endpoint by calling
    /// <c>ResolveEndpoint</c> method on <see cref="GlobalEndpoints.Provider"/> if present, otherwise uses <see cref="AmazonChatbotEndpointProvider"/>.
    /// <para />
    /// Responsible for setting authentication and HTTP headers provided by resolved endpoint.
    /// </summary>
    public class AmazonChatbotEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonChatbotConfig)requestContext.ClientConfig;
            var result = new ChatbotEndpointParameters();
