...
namespace Amazon.Repostspace.Internal
{
    ...
    /// <para />
    /// Custom <see cref="PipelineHandler"/> responsible for resolving endpoint and setting authentication parameters for Repostspace service requests.
    /// <para />
    /// Collects values for <see cref="RepostspaceEndpointParameters"/> and then tries to resolve endpoint by calling
    /// <c>ResolveEndpoint</c> method on <see cref="GlobalEndpoints.Provider"/> if present, otherwise uses <see cref="AmazonRepostspaceEndpointProvider"/>.
    /// <para />
