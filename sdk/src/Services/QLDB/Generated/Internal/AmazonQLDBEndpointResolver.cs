...
namespace Amazon.QLDB.Internal
{
    /// <summary>
    /// Amazon QLDB endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for QLDB service requests.
    /// Collects values for QLDBEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses QLDBEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonQLDBEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonQLDBConfig)requestContext.ClientConfig;
            var result = new QLDBEndpointParameters();
