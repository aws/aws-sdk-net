...
namespace Amazon.Braket.Model
{
    ...
    internal sealed partial class SearchJobsPaginator : IPaginator<SearchJobsResponse>, ISearchJobsPaginator
    {
...
#if BCL
        IEnumerable<SearchJobsResponse> IPaginator<SearchJobsResponse>.Paginate()
        {
            ...
            do
            {
                ...
                response = _client.SearchJobs(_request);
        ...
        async IAsyncEnumerable<SearchJobsResponse> IPaginator<SearchJobsResponse>.PaginateAsync([System.Runtime.CompilerServices.EnumeratorCancellation] CancellationToken cancellationToken)
        {
            if (Interlocked.Exchange(ref _isPaginatorInUse, 1) != 0)
            {
                throw new System.InvalidOperationException("Paginator has already been consumed and cannot be reused. Please create a new instance.");
            ...
            {
                _request.NextToken = nextToken;
                response = await _client.SearchJobsAsync(_request, cancellationToken).ConfigureAwait(false);
                nextToken = response.NextToken;
