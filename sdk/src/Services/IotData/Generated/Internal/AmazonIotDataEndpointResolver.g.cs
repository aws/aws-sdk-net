...
{
    /// <summary>
    /// Amazon IotData endpoint resolver.
    /// <para />
    /// Custom <see cref="PipelineHandler"/> responsible for resolving endpoint and setting authentication parameters for IotData service requests.
    /// <para />
    /// Collects values for <see cref="IotDataEndpointParameters"/> and then tries to resolve endpoint by calling
    /// <c>ResolveEndpoint</c> method on <see cref="GlobalEndpoints.Provider"/> if present, otherwise uses <see cref="AmazonIotDataEndpointProvider"/>.
    /// <para />
    /// Responsible for setting authentication and HTTP headers provided by resolved endpoint.
    /// </summary>
    public class AmazonIotDataEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonIotDataConfig)requestContext.ClientConfig;
            var result = new IotDataEndpointParameters();
