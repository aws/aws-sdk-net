...
{
    /// <summary>
    /// Amazon XRay endpoint resolver.
    /// <para />
    /// Custom <see cref="PipelineHandler"/> responsible for resolving endpoint and setting authentication parameters for XRay service requests.
    /// <para />
    /// Collects values for <see cref="XRayEndpointParameters"/> and then tries to resolve endpoint by calling
    /// <c>ResolveEndpoint</c> method on <see cref="GlobalEndpoints.Provider"/> if present, otherwise uses <see cref="AmazonXRayEndpointProvider"/>.
    /// <para />
    /// Responsible for setting authentication and HTTP headers provided by resolved endpoint.
    /// </summary>
    public class AmazonXRayEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonXRayConfig)requestContext.ClientConfig;
            var result = new XRayEndpointParameters();
