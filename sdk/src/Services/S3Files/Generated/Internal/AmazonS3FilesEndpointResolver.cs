...
using Amazon.Util;
using Amazon.S3Files.Endpoints;

#pragma warning disable 1591

namespace Amazon.S3Files.Internal
{
    /// <summary>
    /// Amazon S3Files endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for S3Files service requests.
    /// Collects values for S3FilesEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses S3FilesEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonS3FilesEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonS3FilesConfig)requestContext.ClientConfig;
            var result = new S3FilesEndpointParameters();
