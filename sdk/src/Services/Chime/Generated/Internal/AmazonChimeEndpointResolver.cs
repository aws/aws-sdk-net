...
using Amazon.Util;
using Amazon.Chime.Endpoints;

#pragma warning disable 1591

namespace Amazon.Chime.Internal
{
    /// <summary>
    /// Amazon Chime endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for Chime service requests.
    /// Collects values for ChimeEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses ChimeEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonChimeEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonChimeConfig)requestContext.ClientConfig;
            var result = new ChimeEndpointParameters();
