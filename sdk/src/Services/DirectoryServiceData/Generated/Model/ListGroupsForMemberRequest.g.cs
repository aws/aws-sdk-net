...
{
    /// <summary>
    /// Container for the parameters to the ListGroupsForMember operation. Returns group information
    /// for the specified member. <para> This operation supports pagination with the use of
    /// the <c>NextToken</c> request and response parameters. If more results are available,
    /// the <c>ListGroupsForMember.NextToken</c> member contains a token that you pass in
    /// the next call to <c>ListGroupsForMember</c>. This retrieves the next set of items.
    /// </para> <para> You can also specify a maximum number of return results with the <c>MaxResults</c>
    /// parameter. </para>
    /// </summary>
