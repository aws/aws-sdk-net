...
{
    /// <summary>
    /// Amazon ResilienceHub endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for ResilienceHub service requests.
    /// Collects values for ResilienceHubEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses ResilienceHubEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonResilienceHubEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonResilienceHubConfig)requestContext.ClientConfig;
            var result = new ResilienceHubEndpointParameters();
