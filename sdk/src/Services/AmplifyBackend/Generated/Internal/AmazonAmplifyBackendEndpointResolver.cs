...
{
    /// <summary>
    /// Amazon AmplifyBackend endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for AmplifyBackend service requests.
    /// Collects values for AmplifyBackendEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses AmplifyBackendEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonAmplifyBackendEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonAmplifyBackendConfig)requestContext.ClientConfig;
            var result = new AmplifyBackendEndpointParameters();
