...
{
    /// <summary>
    /// Amazon SignerData endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for SignerData service requests.
    /// Collects values for SignerDataEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses SignerDataEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonSignerDataEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonSignerDataConfig)requestContext.ClientConfig;
            var result = new SignerDataEndpointParameters();
