...
namespace Amazon.SignerData.Internal
{
    ...
    /// <para />
    /// Custom <see cref="PipelineHandler"/> responsible for resolving endpoint and setting authentication parameters for SignerData service requests.
    /// <para />
    /// Collects values for <see cref="SignerDataEndpointParameters"/> and then tries to resolve endpoint by calling
    /// <c>ResolveEndpoint</c> method on <see cref="GlobalEndpoints.Provider"/> if present, otherwise uses <see cref="AmazonSignerDataEndpointProvider"/>.
    /// <para />
