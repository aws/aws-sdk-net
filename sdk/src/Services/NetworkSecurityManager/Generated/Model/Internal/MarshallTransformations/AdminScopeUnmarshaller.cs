...
namespace Amazon.NetworkSecurityManager.Model.Internal.MarshallTransformations
{
    ...
    public class AdminScopeUnmarshaller : IJsonUnmarshaller<AdminScope, JsonUnmarshallerContext>
    {
        ...
        public AdminScope Unmarshall(JsonUnmarshallerContext context, ref StreamingUtf8JsonReader reader)
        {
            ...
            {
                if (context.TestExpression("firewallTypeScope", targetDepth, ref reader))
                {
                    var unmarshaller = AdminFirewallTypeScopeUnmarshaller.Instance;
                    unmarshalledObject.FirewallTypeScope = unmarshaller.Unmarshall(context, ref reader);
                    continue;
                }
                if (context.TestExpression("scopeFilter", targetDepth, ref reader))
                {
                    var unmarshaller = AdminScopeFilterUnmarshaller.Instance;
                    unmarshalledObject.ScopeFilter = unmarshaller.Unmarshall(context, ref reader);
                    continue;
                }
            }
