...
namespace Amazon.NetworkSecurityManager.Model.Internal.MarshallTransformations
{
    ...
    public class DeletePolicyRequestMarshaller : IMarshaller<IRequest, DeletePolicyRequest> , IMarshaller<IRequest,AmazonWebServiceRequest>
    {
        ...
        public IRequest Marshall(DeletePolicyRequest publicRequest)
        {
            ...
            if (!publicRequest.IsSetPolicyIdentifier())
                throw new AmazonNetworkSecurityManagerException("Request object does not have required field PolicyIdentifier set");
            request.AddPathResource("{policyIdentifier}", StringUtils.FromString(publicRequest.PolicyIdentifier));
            request.ResourcePath = "/policies/{policyIdentifier}";
