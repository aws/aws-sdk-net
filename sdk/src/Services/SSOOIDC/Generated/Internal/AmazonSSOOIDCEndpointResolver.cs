...
{
    /// <summary>
    /// Amazon SSOOIDC endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for SSOOIDC service requests.
    /// Collects values for SSOOIDCEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses SSOOIDCEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonSSOOIDCEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonSSOOIDCConfig)requestContext.ClientConfig;
            var result = new SSOOIDCEndpointParameters();
