...
namespace Amazon.DataPipeline.Internal
{
    /// <summary>
    /// Amazon DataPipeline endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for DataPipeline service requests.
    /// Collects values for DataPipelineEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses DataPipelineEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonDataPipelineEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonDataPipelineConfig)requestContext.ClientConfig;
            var result = new DataPipelineEndpointParameters();
