...
using Amazon.Util;
using Amazon.AccountAccess.Endpoints;
...
{
    /// <summary>
    /// Amazon AccountAccess endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for AccountAccess service requests.
    /// Collects values for AccountAccessEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses AccountAccessEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonAccountAccessEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonAccountAccessConfig)requestContext.ClientConfig;
            var result = new AccountAccessEndpointParameters();
