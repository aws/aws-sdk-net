...
namespace Amazon.CodeGuruProfiler.Model.Internal.MarshallTransformations
{
    ...
    public class SubmitFeedbackRequestMarshaller : IMarshaller<IRequest, SubmitFeedbackRequest> , IMarshaller<IRequest,AmazonWebServiceRequest>
    {
        ...
        public IRequest Marshall(SubmitFeedbackRequest publicRequest)
        {
            ...
            if (!publicRequest.IsSetAnomalyInstanceId())
                throw new AmazonCodeGuruProfilerException("Request object does not have required field AnomalyInstanceId set");
            request.AddPathResource("{anomalyInstanceId}", StringUtils.FromString(publicRequest.AnomalyInstanceId));
            ...
            request.ResourcePath = "/internal/profilingGroups/{profilingGroupName}/anomalies/{anomalyInstanceId}/feedback";
#if !NETFRAMEWORK
