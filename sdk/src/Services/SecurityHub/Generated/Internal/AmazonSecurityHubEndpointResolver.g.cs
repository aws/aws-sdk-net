...
{
    /// <summary>
    /// Amazon SecurityHub endpoint resolver.
    /// <para />
    /// Custom <see cref="PipelineHandler"/> responsible for resolving endpoint and setting authentication parameters for SecurityHub service requests.
    /// <para />
    /// Collects values for <see cref="SecurityHubEndpointParameters"/> and then tries to resolve endpoint by calling
    /// <c>ResolveEndpoint</c> method on <see cref="GlobalEndpoints.Provider"/> if present, otherwise uses <see cref="AmazonSecurityHubEndpointProvider"/>.
    /// <para />
    /// Responsible for setting authentication and HTTP headers provided by resolved endpoint.
    /// </summary>
    public class AmazonSecurityHubEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonSecurityHubConfig)requestContext.ClientConfig;
            var result = new SecurityHubEndpointParameters();
