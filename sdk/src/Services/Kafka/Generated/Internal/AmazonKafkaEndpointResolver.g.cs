...
using Amazon.Util;
using Amazon.Kafka.Endpoints;

#pragma warning disable 1591

namespace Amazon.Kafka.Internal
{
    /// <summary>
    /// Amazon Kafka endpoint resolver.
    /// <para />
    /// Custom <see cref="PipelineHandler"/> responsible for resolving endpoint and setting authentication parameters for Kafka service requests.
    /// <para />
    /// Collects values for <see cref="KafkaEndpointParameters"/> and then tries to resolve endpoint by calling
    /// <c>ResolveEndpoint</c> method on <see cref="GlobalEndpoints.Provider"/> if present, otherwise uses <see cref="AmazonKafkaEndpointProvider"/>.
    /// <para />
    /// Responsible for setting authentication and HTTP headers provided by resolved endpoint.
    /// </summary>
    public class AmazonKafkaEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonKafkaConfig)requestContext.ClientConfig;
            var result = new KafkaEndpointParameters();
