...
namespace Amazon.NetworkFirewall.Internal
{
    /// <summary>
    /// Amazon NetworkFirewall endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for NetworkFirewall service requests.
    /// Collects values for NetworkFirewallEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses NetworkFirewallEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonNetworkFirewallEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonNetworkFirewallConfig)requestContext.ClientConfig;
            var result = new NetworkFirewallEndpointParameters();
