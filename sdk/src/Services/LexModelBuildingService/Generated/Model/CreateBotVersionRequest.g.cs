...
{
    /// <summary>
    /// Container for the parameters to the CreateBotVersion operation. Creates a new version
    /// of the bot based on the <c>$LATEST</c> version. If the <c>$LATEST</c> version of this
    /// resource hasn't changed since you created the last version, Amazon Lex doesn't create
    /// a new version. It returns the last created version. <note> <para> You can update only
    /// the <c>$LATEST</c> version of the bot. You can't update the numbered versions that
    /// you create with the <c>CreateBotVersion</c> operation. </para> </note> <para> When
    /// you create the first version of a bot, Amazon Lex sets the version to 1. Subsequent
    /// versions increment by 1. For more information, see <a>versioning-intro</a>. </para>
    /// <para> This operation requires permission for the <c>lex:CreateBotVersion</c> action.
    /// </para>
    /// </summary>
