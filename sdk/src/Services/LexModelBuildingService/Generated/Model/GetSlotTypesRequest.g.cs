...
{
    /// <summary>
    /// Container for the parameters to the GetSlotTypes operation. Returns slot type information
    /// as follows: <ul> <li> <para> If you specify the <c>nameContains</c> field, returns
    /// the <c>$LATEST</c> version of all slot types that contain the specified string. </para>
    /// </li> <li> <para> If you don't specify the <c>nameContains</c> field, returns information
    /// about the <c>$LATEST</c> version of all slot types. </para> </li> </ul> <para> The
    /// operation requires permission for the <c>lex:GetSlotTypes</c> action. </para>
    /// </summary>
