...
{
    /// <summary>
    /// Container for the parameters to the CreateIntentVersion operation. Creates a new version
    /// of an intent based on the <c>$LATEST</c> version of the intent. If the <c>$LATEST</c>
    /// version of this intent hasn't changed since you last updated it, Amazon Lex doesn't
    /// create a new version. It returns the last version you created. <note> <para> You can
    /// update only the <c>$LATEST</c> version of the intent. You can't update the numbered
    /// versions that you create with the <c>CreateIntentVersion</c> operation. </para> </note>
    /// <para> When you create a version of an intent, Amazon Lex sets the version to 1. Subsequent
    /// versions increment by 1. For more information, see <a>versioning-intro</a>. </para>
    /// <para> This operation requires permissions to perform the <c>lex:CreateIntentVersion</c>
    /// action. </para>
    /// </summary>
