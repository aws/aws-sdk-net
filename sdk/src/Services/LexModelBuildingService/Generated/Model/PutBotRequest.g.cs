...
{
    /// <summary>
    /// Container for the parameters to the PutBot operation. Creates an Amazon Lex conversational
    /// bot or replaces an existing bot. When you create or update a bot you are only required
    /// to specify a name, a locale, and whether the bot is directed toward children under
    /// age 13. You can use this to add intents later, or to remove intents from an existing
    /// bot. When you create a bot with the minimum information, the bot is created or updated
    /// but Amazon Lex returns the <c></c> response <c>FAILED</c>. You can build the bot after
    /// you add one or more intents. For more information about Amazon Lex bots, see <a>how-it-works</a>.
    /// <para> If you specify the name of an existing bot, the fields in the request replace
    /// the existing values in the <c>$LATEST</c> version of the bot. Amazon Lex removes any
    /// fields that you don't provide values for in the request, except for the <c>idleTTLInSeconds</c>
    /// and <c>privacySettings</c> fields, which are set to their default values. If you don't
    /// specify values for required fields, Amazon Lex throws an exception. </para> <para>
    /// This operation requires permissions for the <c>lex:PutBot</c> action. For more information,
    /// see <a>security-iam</a>. </para>
    /// </summary>
    ...
    public partial class PutBotRequest : AmazonLexModelBuildingServiceRequest
    {
        ...
        /// <summary>
        /// Checks to see if the ProcessBehavior property is set.
        /// </summary>
        internal bool IsSetProcessBehavior() => this.ProcessBehavior != null;
