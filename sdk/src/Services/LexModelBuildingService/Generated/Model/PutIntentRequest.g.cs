...
{
    /// <summary>
    /// Container for the parameters to the PutIntent operation. Creates an intent or replaces
    /// an existing intent. <para> To define the interaction between the user and your bot,
    /// you use one or more intents. For a pizza ordering bot, for example, you would create
    /// an <c>OrderPizza</c> intent. </para> <para> To create an intent or replace an existing
    /// intent, you must provide the following: </para> <ul> <li> <para> Intent name. For
    /// example, <c>OrderPizza</c>. </para> </li> <li> <para> Sample utterances. For example,
    /// "Can I order a pizza, please." and "I want to order a pizza." </para> </li> <li> <para>
    ...
    /// a time, or custom slot types such as the size and crust of a pizza. </para> </li>
    /// <li> <para> How the intent will be fulfilled. You can provide a Lambda function or
    /// configure the intent to return the intent information to the client application. If
    /// you use a Lambda function, when all of the intent information is available, Amazon
    /// Lex invokes your Lambda function. If you configure your intent to return the intent
    /// information to the client application. </para> </li> </ul> <para> You can specify
    /// other optional information in the request, such as: </para> <ul> <li> <para> A confirmation
    /// prompt to ask the user to confirm an intent. For example, "Shall I order your pizza?"
    /// </para> </li> <li> <para> A conclusion statement to send to the user after the intent
    /// has been fulfilled. For example, "I placed your pizza order." </para> </li> <li> <para>
    /// A follow-up prompt that asks the user for additional activity. For example, asking
    /// "Do you want to order a drink with your pizza?" </para> </li> </ul> <para> If you
    /// specify an existing intent name to update the intent, Amazon Lex replaces the values
    /// in the <c>$LATEST</c> version of the intent with the values in the request. Amazon
    /// Lex removes fields that you don't provide in the request. If you don't specify the
    ...
    /// version of the intent is set to <c>NOT_BUILT</c>. </para> <para> For more information,
    /// see <a>how-it-works</a>. </para> <para> This operation requires permissions for the
    /// <c>lex:PutIntent</c> action. </para>
    /// </summary>
