...
{
    /// <summary>
    /// Amazon Signer endpoint resolver.
    /// <para />
    /// Custom <see cref="PipelineHandler"/> responsible for resolving endpoint and setting authentication parameters for Signer service requests.
    /// <para />
    /// Collects values for <see cref="SignerEndpointParameters"/> and then tries to resolve endpoint by calling
    /// <c>ResolveEndpoint</c> method on <see cref="GlobalEndpoints.Provider"/> if present, otherwise uses <see cref="AmazonSignerEndpointProvider"/>.
    /// <para />
    /// Responsible for setting authentication and HTTP headers provided by resolved endpoint.
    /// </summary>
    public class AmazonSignerEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonSignerConfig)requestContext.ClientConfig;
            var result = new SignerEndpointParameters();
