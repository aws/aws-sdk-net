...
namespace Amazon.AuditManager.Model
{
    ...
    /// Audit Manager. <para> When you remove a delegated administrator from your Audit Manager
    /// settings, you continue to have access to the evidence that you previously collected
    /// under that account. This is also the case when you deregister a delegated administrator
    /// from Organizations. However, Audit Manager stops collecting and attaching evidence
    /// to that delegated administrator account moving forward. </para> <important> <para>
    /// Keep in mind the following cleanup task if you use evidence finder: </para> <para>
    ...
    /// the event data store</a>. </para> <para> This cleanup task is necessary to ensure
    /// that you don't end up with multiple event data stores. Audit Manager ignores an unused
    /// event data store after you remove or change a delegated administrator account. However,
    /// the unused event data store continues to incur storage costs from CloudTrail Lake
    /// if you don't delete it. </para> </important> <para> When you deregister a delegated
    /// administrator account for Audit Manager, the data for that account isn’t deleted.
    /// If you want to delete resource data for a delegated administrator account, you must
    /// perform that task separately before you deregister the account. Either, you can do
    /// this in the Audit Manager console. Or, you can use one of the delete API operations
    /// that are provided by Audit Manager. </para> <para> To delete your Audit Manager resource
    /// data, see the following instructions: </para> <ul> <li> <para> <a href="https://docs.aws.amazon.com/audit-manager/latest/APIReference/API_DeleteAssessment.html">DeleteAssessment</a>
    /// (see also: <a href="https://docs.aws.amazon.com/audit-manager/latest/userguide/delete-assessment.html">Deleting
    /// an assessment</a> in the <i>Audit Manager User Guide</i>) </para> </li> <li> <para>
    ...
    /// a custom framework</a> in the <i>Audit Manager User Guide</i>) </para> </li> <li>
    /// <para> <a href="https://docs.aws.amazon.com/audit-manager/latest/APIReference/API_DeleteAssessmentFrameworkShare.html">DeleteAssessmentFrameworkShare</a>
    ...
    /// a share request</a> in the <i>Audit Manager User Guide</i>) </para> </li> <li> <para>
    ...
    /// an assessment report</a> in the <i>Audit Manager User Guide</i>) </para> </li> <li>
    /// <para> <a href="https://docs.aws.amazon.com/audit-manager/latest/APIReference/API_DeleteControl.html">DeleteControl</a>
    /// (see also: <a href="https://docs.aws.amazon.com/audit-manager/latest/userguide/delete-controls.html">Deleting
    /// a custom control</a> in the <i>Audit Manager User Guide</i>) </para> </li> </ul> <para>
