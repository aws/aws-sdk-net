...
//        /// <returns></returns>
//        public static async Task<bool> DoesS3BucketExistAsync(IAmazonS3 s3Client, string bucketName)
//        {
//            if (s3Client == null)
//            {
...
//        /// <param name="s3Client">The Amazon S3 Client to use for S3 specific operations.</param>
//        /// <seealso cref="T:Amazon.S3.S3StorageClass"/>
//        public static Task SetObjectStorageClassAsync(IAmazonS3 s3Client, string bucketName, string key, S3StorageClass sClass)
//        {
//              return SetObjectStorageClassAsync(s3Client, bucketName, key, null, sClass);
//        }
...
//        /// <param name="s3Client">The Amazon S3 Client to use for S3 specific operations.</param>
//        /// <seealso cref="T:Amazon.S3.S3StorageClass"/>
//        public static async Task SetObjectStorageClassAsync(IAmazonS3 s3Client, string bucketName, string key, string version, S3StorageClass sClass)
//        {
//            AsyncReferenceHolder hol = new AsyncReferenceHolder();
//            await SetupForObjectModificationAsync(s3Client, bucketName, key, version, hol).ConfigureAwait(false);

//            hol.copyRequest.StorageClass = sClass;
//            CopyObjectResponse copyResponse = await s3Client.CopyObjectAsync(hol.copyRequest).ConfigureAwait(false);

//            if (!string.IsNullOrEmpty(copyResponse.SourceVersionId))
//                hol.putACLRequest.VersionId = copyResponse.SourceVersionId;

//            await s3Client.PutACLAsync(hol.putACLRequest).ConfigureAwait(false);
//        }
...
//        /// <param name="method">The server side encryption method</param>
//        /// <param name="s3Client">The Amazon S3 Client to use for S3 specific operations.</param>
//        public static Task SetServerSideEncryptionAsync(IAmazonS3 s3Client, string bucketName, string key, ServerSideEncryptionMethod method)
//        {
//            return SetServerSideEncryptionAsync(s3Client, bucketName, key, null, method);
//        }
...
//        /// <param name="method">The server side encryption method</param>
//        /// <param name="s3Client">The Amazon S3 Client to use for S3 specific operations.</param>
//        public static async Task SetServerSideEncryptionAsync(IAmazonS3 s3Client, string bucketName, string key, string version, ServerSideEncryptionMethod method)
//        {
//            AsyncReferenceHolder hol = new AsyncReferenceHolder();
//            await SetupForObjectModificationAsync(s3Client, bucketName, key, version, hol).ConfigureAwait(false);

//            hol.copyRequest.ServerSideEncryptionMethod = method;
//            CopyObjectResponse copyResponse = await s3Client.CopyObjectAsync(hol.copyRequest).ConfigureAwait(false);
...
//        /// <param name="websiteRedirectLocation">The redirect location</param>
//        /// <param name="s3Client">The Amazon S3 Client to use for S3 specific operations.</param>
//        public static async Task SetWebsiteRedirectLocationAsync(IAmazonS3 s3Client, string bucketName, string key, string websiteRedirectLocation)
//        {
//            AsyncReferenceHolder hol = new AsyncReferenceHolder();

//            await SetupForObjectModificationAsync(s3Client, bucketName, key, null, hol).ConfigureAwait(false);

//            hol.copyRequest.WebsiteRedirectLocation = websiteRedirectLocation;
//            CopyObjectResponse copyResponse = await s3Client.CopyObjectAsync(hol.copyRequest).ConfigureAwait(false);
...
//        static async Task<AsyncReferenceHolder> SetupForObjectModificationAsync(IAmazonS3 s3Client, string bucketName, string key, string version 
//           ,AsyncReferenceHolder hol)
//        {
...
//            GetACLResponse getACLResponse = await s3Client.GetACLAsync(getACLRequest).ConfigureAwait(false);


//            // Set the object's original ACL back onto it because a COPY
//            // operation resets the ACL on the destination object.
//            hol.putACLRequest = new PutACLRequest();
//            hol.putACLRequest.BucketName = bucketName;
//            hol.putACLRequest.Key = key;
//            hol.putACLRequest.AccessControlList = getACLResponse.AccessControlList;


//            ListObjectsResponse listObjectResponse = await s3Client.ListObjectsAsync(new ListObjectsRequest
//            {
//                BucketName = bucketName,
//                Prefix = key,
//                MaxKeys = 1
...
//                Key = key
//            };
//            GetObjectMetadataResponse getMetaResponse = await s3Client.GetObjectMetadataAsync(getMetaRequest).ConfigureAwait(false);

//            // Set the storage class on the object
//            hol.copyRequest = new CopyObjectRequest();
//            hol.copyRequest.SourceBucket = hol.copyRequest.DestinationBucket = bucketName;
//            hol.copyRequest.SourceKey = hol.copyRequest.DestinationKey = key;
//            hol.copyRequest.StorageClass = listObjectResponse.S3Objects[0].StorageClass == "STANDARD" ? S3StorageClass.Standard : S3StorageClass.ReducedRedundancy;
//            if (version != null)
//                hol.copyRequest.SourceVersionId = version;

//            hol.copyRequest.WebsiteRedirectLocation = getMetaResponse.WebsiteRedirectLocation;
//            hol.copyRequest.ServerSideEncryptionMethod = getMetaResponse.ServerSideEncryptionMethod;
//            return hol;
//        }
...
//        /// <param name="s3Client">The Amazon S3 Client to use for S3 specific operations.</param>
//        /// 1
//        public static async Task DeleteS3BucketWithObjectsAsync(IAmazonS3 s3Client, string bucketName)
//        {
//            var result = await BeginDeleteS3BucketWithObjectsAsync(s3Client, bucketName, null, null).ConfigureAwait(false);
...
//        /// 2
//        public static async Task DeleteS3BucketWithObjectsAsync(IAmazonS3 s3Client, string bucketName, S3DeleteBucketWithObjectsOptions deleteOptions)
//        {
//            var result = await BeginDeleteS3BucketWithObjectsAsync(s3Client, bucketName, deleteOptions, null, null).ConfigureAwait(false);
//            EndDeleteS3BucketWithObjects(result);
//        }
...
//        /// 3
//        public static async Task<IAsyncCancelableResult> BeginDeleteS3BucketWithObjectsAsync(IAmazonS3 s3Client,string bucketName, 
//            AsyncCallback callback, object state)
//        {
//            return await BeginDeleteS3BucketWithObjectsAsync(s3Client, bucketName, 
...
//        /// be used to cancel the operation while it's in progress.</returns>
//        /// 4
//        public static Task<IAsyncCancelableResult> BeginDeleteS3BucketWithObjectsAsync(IAmazonS3 s3Client, string bucketName, 
//             S3DeleteBucketWithObjectsOptions deleteOptions, AsyncCallback callback, object state)
//        {
//            return BeginDeleteS3BucketWithObjectsAsync(s3Client, bucketName, deleteOptions, null, callback, state);
//        }
...
//                S3Client = s3Client
//            };
//            await InvokeDeleteS3BucketWithObjects(request).ConfigureAwait(false);
//            return asyncResult;
//        }
...
//        private static async Task InvokeDeleteS3BucketWithObjects(object state)
//        {
//            var request = (S3DeleteBucketWithObjectsRequest)state;

//            try
//            {
//                await DeleteS3BucketWithObjectsInternalAsync(
//                    request.S3Client,
//                    request.BucketName,
...
//        public static async Task<S3PostUploadResponse> PostUploadAsync(S3PostUploadRequest request)
//        {
//            string url;
...
//            using (var reqStream = await webRequest.GetRequestStreamAsync().ConfigureAwait(false))
//            {
...
//            try
//            {
//                response = (await webRequest.GetResponseAsync().ConfigureAwait(false)) as HttpWebResponse;
//            }
...
//        private class AsyncReferenceHolder
//        {
//            public CopyObjectRequest copyRequest;
//            public PutACLRequest putACLRequest;
//        }
//    }
//}
