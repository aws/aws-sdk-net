...
namespace Amazon.S3.Model.Internal.MarshallTransformations
{
    ...
    /// </summary>  
    public class GetBucketWebsiteResponseUnmarshaller : XmlResponseUnmarshaller
    {
        /// <summary>
        ...
        private static void UnmarshallResult(XmlUnmarshallerContext context, GetBucketWebsiteResponse response)
        {
            ...
            while (context.Read())
            {
                if (context.IsStartElement || context.IsAttribute)
                {
                    ...
                    {
                        var unmarshaller = ErrorDocumentUnmarshaller.Instance;
                        response.ErrorDocument = unmarshaller.Unmarshall(context);
                        continue;
                    }
                    if (context.TestExpression("IndexDocument", targetDepth))
                    {
                        var unmarshaller = IndexDocumentUnmarshaller.Instance;
                        response.IndexDocument = unmarshaller.Unmarshall(context);
                        continue;
                    }
                    if (context.TestExpression("RedirectAllRequestsTo", targetDepth))
                    {
                        var unmarshaller = RedirectAllRequestsToUnmarshaller.Instance;
                        response.RedirectAllRequestsTo = unmarshaller.Unmarshall(context);
                        continue;
                    }
                    if (context.TestExpression("RoutingRules/RoutingRule", targetDepth))
                    {
                        if (response.RoutingRules == null)
                        {
                            response.RoutingRules = new List<RoutingRule>();
                        }
                        var unmarshaller = RoutingRuleUnmarshaller.Instance;
                        response.RoutingRules.Add(unmarshaller.Unmarshall(context));
                        continue;
                    }
                }
