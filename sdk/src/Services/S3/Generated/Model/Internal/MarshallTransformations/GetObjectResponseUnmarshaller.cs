...
namespace Amazon.S3.Model.Internal.MarshallTransformations
{
    ...
    /// </summary>  
    public class GetObjectResponseUnmarshaller : XmlResponseUnmarshaller
    {
        /// <summary>
        ...
        public override AmazonWebServiceResponse Unmarshall(XmlUnmarshallerContext context)
        {
            ...
            var headersForMetadata = new Dictionary<string, string>();
            foreach (var nameMetadata in context.ResponseData.GetHeaderNames())
            {
                var keyToUse = nameMetadata;
                if("x-amz-meta-".Length > 0 && keyToUse.StartsWith("x-amz-meta-")) {
                    keyToUse = keyToUse.Substring("x-amz-meta-".Length);
                }

                if (context.ResponseData.IsHeaderPresent($"x-amz-meta-{keyToUse}"))
                {
                    headersForMetadata.Add(
                        keyToUse,
                        context.ResponseData.GetHeaderValue($"x-amz-meta-{keyToUse}")
                    );
                }
            }
            if(headersForMetadata.Count > 0)
                response.Metadata = headersForMetadata;
