...
{
    /// <summary>
    /// Container for the parameters to the CreateDataflowEndpointGroup operation. Creates
    /// a <c>DataflowEndpoint</c> group containing the specified list of <c> DataflowEndpoint</c>
    /// objects. <para> The <c>name</c> field in each endpoint is used in your mission profile
    /// <c> DataflowEndpointConfig</c> to specify which endpoints to use during a contact.
    /// </para> <para> When a contact uses multiple <c>DataflowEndpointConfig</c> objects,
    /// each <c> Config</c> must match a <c>DataflowEndpoint</c> in the same group. </para>
    /// </summary>
    public partial class CreateDataflowEndpointGroupRequest : AmazonGroundStationRequest
    {
        ...
        public List<EndpointDetails> EndpointDetails { get; set; } = AWSConfigs.InitializeCollections ? new List<EndpointDetails>() : null;

        /// <summary>
        /// Checks to see if the EndpointDetails property is set.
        /// </summary>
        internal bool IsSetEndpointDetails() => this.EndpointDetails != null && (this.EndpointDetails.Count > 0 || !AWSConfigs.InitializeCollections);
