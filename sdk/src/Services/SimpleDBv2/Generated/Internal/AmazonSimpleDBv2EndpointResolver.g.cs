...
namespace Amazon.SimpleDBv2.Internal
{
    /// <summary>
    /// Amazon SimpleDBv2 endpoint resolver.
    /// <para />
    /// Custom <see cref="PipelineHandler"/> responsible for resolving endpoint and setting authentication parameters for SimpleDBv2 service requests.
    /// <para />
    /// Collects values for <see cref="SimpleDBv2EndpointParameters"/> and then tries to resolve endpoint by calling
    /// <c>ResolveEndpoint</c> method on <see cref="GlobalEndpoints.Provider"/> if present, otherwise uses <see cref="AmazonSimpleDBv2EndpointProvider"/>.
    /// <para />
    /// Responsible for setting authentication and HTTP headers provided by resolved endpoint.
    /// </summary>
    public class AmazonSimpleDBv2EndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonSimpleDBv2Config)requestContext.ClientConfig;
            var result = new SimpleDBv2EndpointParameters();
