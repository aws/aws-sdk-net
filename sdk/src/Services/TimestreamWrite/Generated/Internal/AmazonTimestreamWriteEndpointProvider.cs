...
namespace Amazon.TimestreamWrite.Internal
{
    /// <summary>
    /// Amazon TimestreamWrite endpoint provider.
    /// Resolves endpoint for given set of TimestreamWriteEndpointParameters.
    /// Can throw AmazonClientException if endpoint resolution is unsuccessful.
    /// </summary>
    public class AmazonTimestreamWriteEndpointProvider : IEndpointProvider
    {
        /// <summary>
        /// Resolve endpoint for TimestreamWriteEndpointParameters
        /// </summary>
                ...
                if ((refs["PartitionResult"] = Partition((string)refs["Region"])) != null)
                {
                    ...
                    if (Equals(refs["UseFIPS"], true))
                    {
                        if (Equals(GetAttr(refs["PartitionResult"], "supportsFIPS"), true))
                        {
                            ...
                            {
                                return new Endpoint(Interpolate(@"https://ingest.timestream.{Region}.amazonaws.com", refs), InterpolateJson(@"", refs), InterpolateJson(@"", refs));
                            }
                            return new Endpoint(Interpolate(@"https://ingest.timestream-fips.{Region}.{PartitionResult#dnsSuffix}", refs), InterpolateJson(@"", refs), InterpolateJson(@"", refs));
                        }
                    ...
                    return new Endpoint(Interpolate(@"https://ingest.timestream.{Region}.{PartitionResult#dnsSuffix}", refs), InterpolateJson(@"", refs), InterpolateJson(@"", refs));
                }
            }
