...
namespace Amazon.Runtime.Internal.Auth
{
    ...
    public class AWS4EventSigner : IEventSigner, IDisposable
    {
        ...
        public async Task<byte[]> SignEventAsync(byte[] eventBytes)
        {
            ...
            {
                _logger.Error(ex,
                    "AWS4EventSigner.SignEventAsync FAILED: instanceId={0}, eventCount={1}, credentialType={2}, " +
                    "previousSignature={3}, initialRequestSignature={4}",
                    _instanceId,
                    _eventCount,
                    _credentials?.GetType().Name ?? "null",
                    _previousSignature?.Substring(0, Math.Min(16, _previousSignature?.Length ?? 0)) + "...",
                    _initialRequestSignature?.Substring(0, Math.Min(16, _initialRequestSignature?.Length ?? 0)) + "...");
                throw;
            }
            finally
            {
