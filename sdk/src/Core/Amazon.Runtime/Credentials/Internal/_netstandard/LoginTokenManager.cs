            ...
            if (cachedToken.Success)
            {
                ...
                else if (loginToken.CanRefresh())
                {
                    ...
                    try
                    {
                        ...
                        {
                            throw new ArgumentNullException(nameof(loginToken.DPoPKey), $"DPoP private key missing from the cached Login Token for login session [{options.LoginSession}]. Please reauthenticate using AWS CLI or AWS Tools for PowerShell.");
                        }
