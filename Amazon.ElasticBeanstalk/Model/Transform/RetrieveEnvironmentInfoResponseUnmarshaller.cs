...
namespace Amazon.ElasticBeanstalk.Model.Transform
{
    ...
    /// </summary>
    internal class RetrieveEnvironmentInfoResponseUnmarshaller : IResponseUnmarshaller<RetrieveEnvironmentInfoResponse, UnmarshallerContext> {

        public RetrieveEnvironmentInfoResponse Unmarshall(UnmarshallerContext context) 
        {
            RetrieveEnvironmentInfoResponse response = new RetrieveEnvironmentInfoResponse();

            while (context.Read())
            {
                if (context.IsStartElement)
                {
                    if(context.TestExpression("RetrieveEnvironmentInfoResult", 2))
                    {
                        response.RetrieveEnvironmentInfoResult = RetrieveEnvironmentInfoResultUnmarshaller.GetInstance().Unmarshall(context);
                        continue;
                    }
        ...
        public AmazonServiceException UnmarshallException(UnmarshallerContext context, Exception innerException, HttpStatusCode statusCode)
        {
            ...
            if (errorResponse.Code != null && errorResponse.Code.Equals("ValidationErrorException"))
            {
